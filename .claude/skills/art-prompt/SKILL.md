...
프롬프트 첫머리에서 입력별 역할을 구분한다: **외형 참조는 얼굴·머리·피부색·장비 실루엣·강조색을 보존**하고, **그림체 참조는 선·음영·비율·프레이밍만 따른다.** 둘을 섞어 새 얼굴을 임의 발명하지 않는다.
