...
- **기본형 그대로** 쓴다 — 소재와 1~9번 타일을 전부 채운다. 사용자가 지정한 인물을 먼저 배치하고, 남는 칸은 「삼지창을 든 나가」처럼 같은 종족·집단의 짧고 다른 아이디어로 채운다. 원형 블록(§4)은 2줄을 넘기지 않고, **금지어 표(§2)를 대조**한다 — `pauldron` · `rust` · `studded` · `gorget` … 한 단어가 지표 하나를 무너뜨린다
- **색은 비운다** — hex 를 쓰지 않는다. 사용자가 정한 컨셉 색(「붉은 데몬」 · 「청동 우상」)만 단어로 남긴다. 초록 계약 줄은 그대로 둔다
- 아홉 타일의 **실루엣 축을 한 줄로 적을 수 있는지** 확인한다(원칙 6). 못 적으면 아홉이 닮게 나온다
