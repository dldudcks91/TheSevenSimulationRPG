...
**생성 전에 프로젝트의 기존 초상을 실제로 열어 보는 것이 필수다** [2026-09-24 사용자]. 텍스트 설명이나 파일명만 보고 그리지 않는다. 기본 앵커가 생성기에 이미 붙어 있어도, 이번 대상과 직접 이어지는 프로젝트 그림을 별도 입력으로 고른다.

- 재발주: 그 대상의 현행 `faces/source/ready/` 원본을 직접 확인한다. 외형을 보존할 때는 **외형 참조**로 첨부한다. 사용자가 기존 얼굴·그림결을 버리고 재설계를 요청했다면 직업·장비만 확인하고, 생성 입력은 새 그림체 참조에 집중한다.
- 신규: 같은 종족을 최우선으로, 같은 챕터·진영·역할 순으로 가까운 기존 초상을 고른다.
- 모든 발주: 최소 **외형 참조 1장 + 그림체 참조 1장**. 적절한 참조가 없으면 생성하지 않고 먼저 보고한다.
- 입력별 역할을 프롬프트에 적는다. 외형 참조는 사용자가 보존하라고 한 얼굴·장비·강조색을 나르고, 그림체 참조는 비율·외곽선·음영·프레이밍·디테일 생략 방식만 나른다. 그림체 참조 인물의 얼굴·옷·소품을 새 캐릭터에게 복제하지 않는다.

아래 표는 어떤 그림을 어떤 역할로 붙일지 고를 때 본다.
...
**첨부가 곧 외형·스타일 지시다** — 기존 그림을 직접 보고 붙일 그림을 고르는 데 시간을 쓰고, 글로 새 생김새를 발명하는 데는 쓰지 않는다.

## 1. 스케치 골격 — 기본형 (공통 지시 8줄 이내 + 소재 9줄)
...
[소재 — 1~2줄. 원형 + 아홉을 가르는 축 한 줄. 색은 비운다]

1. [요청 종족/집단 + 짧고 구체적인 시각 소재]
2. [요청 종족/집단 + 다른 소재]
3. [요청 종족/집단 + 다른 소재]
4. [요청 종족/집단 + 다른 소재]
5. [요청 종족/집단 + 다른 소재]
...
**매번 1~9번을 전부 채운다.** 사용자가 지정한 인물을 먼저 배치하고, 남는 번호는 같은 종족·집단에서 얼굴·장비·문화가 다른 인물로 창의적으로 채운다. 각 줄은 「삼지창을 든 나가」 정도로 짧게 쓰고, 스타일·팔레트·디테일을 다시 지시하지 않는다. 번호는 발주문 정리용이며 그림에 넣지 않는다.
...
**색은 비운다** [2026-09-14 사용자] — 배경·격자 계약 외에는 hex 를 쓰지 않는다. 사용자가 정한 컨셉 색만 단어로 남긴다(「붉은 데몬」 · 「청동」). 뽑을 때마다 달라지는 것이 스케치의 목적이다 — 아홉이 같은 색으로 몰려 나올 때만 재발주에서 타일 색을 구체화한다.
...
소재 줄은 「무엇을 그릴지 + 아홉을 가르는 축」만이다. 분위기·디테일·팔레트는 앵커가 나르므로 적지 않는다. 아래 `Four` 예시는 과거 2×2 발주 기록이다. 현재 기본형에는 종족·인물 수를 3×3·아홉으로 바꿔 적용한다.
...
- **한 세트 = 축 하나.** 판금은 머리 모양, 맨머리는 머리카락, 후드는 후드 형태(뾰족/둥글고 뒤로/낮고 평평/비대칭). 축이 둘 이상이면 세트가 흩어지고, 없으면 같은 사람이 아홉 나온다
- **타일 한 줄은 한 마디** — 실루엣 하나. 두 줄을 넘으면 그 타일이 앵커를 이긴다(§1)
- **색은 비운다**(§1) — 아홉이 같은 색으로 몰려 나오면 그때 재발주에서 타일 색을 구체화한다
- 사용자가 **같은 인물의 색 변형 4종**을 요구하면 얼굴·자세를 고정하고 후드/망토 색만 hex 로 갈라 준다(색상환을 벌린다 — 검정 · 적갈 · 청회 · 카키)
- **현재 3×3의 아홉 칸을 모두 검수한다.** 과거 2×2 시트의 4번 워터마크 사례를 현재 기본형에 적용하지 않는다
...
아래의 2×2는 과거 세부 모드 예시다. 새 요청에서 별도 장수를 정하지 않았으면 현재 기본 3×3·아홉 칸 규칙을 적용한다.
