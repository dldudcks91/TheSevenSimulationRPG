...
- 재발주: 그 대상의 현행 `faces/source/ready/` 원본을 **외형 참조**로 반드시 첨부한다.
- 신규: 같은 종족을 최우선으로, 같은 챕터·진영·역할 순으로 가까운 기존 초상을 고른다.
- 모든 발주: 최소 **외형 참조 1장 + 그림체 참조 1장**. 적절한 참조가 없으면 생성하지 않고 먼저 보고한다.
- 입력별 역할을 프롬프트에 적는다. 외형 참조는 얼굴형·눈·머리·피부색·장비 실루엣·강조색을 나르고, 그림체 참조는 비율·외곽선·음영·프레이밍을 나른다.
...
| 같은 대상의 현행 초상 | `faces/source/ready/<hero|monster>/<id>.png` | **외형 보존의 필수 입력** — 재발주 때 생략 금지 |
