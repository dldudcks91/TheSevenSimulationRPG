...
---
*마지막 업데이트: 2026-09-09 ([skill_tiles.md](skill_tiles.md) 신설 — 직업 스킬 풀 37개(`skill_design.md §12`)의 **타일 문장**을 모으는 자리. 전사 7 확정. ⚠ 그 세트는 **2차 시트의 단색 실루엣**으로 가므로 이 문서의 스킬 합격선(색 수 110~160 · 경계 12~21%)과 앵커 표(`example_1.png`)가 안 맞는다 — 단색 합격선 재정의는 미착수) · 2026-09-07 (최초 작성 — 설치본 17장 + empty 5장 실측(색 수·경계밀도·외곽선·0.88 프레이밍)으로 합격선을 세우고, README 누끼 규칙을 cut.py 로 실행 가능하게 옮겨 원본 시트 3장 전부에서 설치본 재현을 검증. 발주 프롬프트는 미문서화 상태였어서 앵커 실측에서 역산. 사용자 지시)*
