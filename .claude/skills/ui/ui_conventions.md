...
- **앱 시계가 부르는 그리기만 예외로 미룬다** (2026-09-24 · ADR-0338) — 라운드 정산 · 런 끝 · 반복의 다음 런처럼 **유저가 누르지 않았는데** 그려야 하면 `render()` 대신 `clockRender = true` 를 세운다. 다음 앱 시계 눈금이 툴팁 · 누름이 없을 때 한 번 그린다. 관전 화면이면 재생기를 걷지 않고 보관 칸만 — `bagStale = true`(골드 숫자는 `refreshGold` 로 곧바로 · ADR-0341). 유저 조작의 결과는 지금처럼 곧바로 `render()`
...
- 화면 상태는 `app.js` 의 `state` 객체 하나다 (`screen` · `tab` · `exp` · `heroUid` · `codexChapter` · `slotFilter` · `roll` · `candidates` · `confirmOverwrite` · `bagSelMode` · `bagSel` · `bagLockMode` · `bagSortMode` · `battle`). **세이브에 들어가지 않는다** — 새 화면 상태도 여기 붙인다
...
- 섹션 주석 `/* ───── 이름 ───── */` 순서: 게임 화면 기본기 · 셸 · 공통 조각 · 원정 · 로스터 · 장비 · 리포트 · 전투 관전 · 행동 게이지/쿨 게이지 · 서브 바/세그먼트 · 스킬 · 선술집 · 도감 · **도감 — 자산 세그먼트**(`.ix-*` · 옛 「이미지 도감」) · 페이퍼돌 · 인벤토리 격자 · 접이식 · 캐릭터 탭 · 새 게임 · 초반 루프 실동작 · 도움말 탭. 새 규칙은 해당 섹션 **안에** 넣는다
- **게임 UI 기본기 — 지우지 않는다.** 문서가 아니라 게임 화면이라 셋을 따로 꺼 둔다: 드래그 선택(`user-select`) · 캐럿(`caret-color`) · 마우스 포커스 링(`:focus` 는 끄고 `:focus-visible` 만 남겨 키보드 이동을 보존). 원인이 각각 달라 하나만 꺼서는 안 잡힌다. **스크롤바도 전체 선택자 하나가 끈다** (ADR-0097) — 규칙 안에 `scrollbar-width` 를 적으면 클래스 선택자가 이겨 막대가 되살아난다
...
- `mountBattle(container, opts)` 가 정리 함수를 돌려준다. 렌더러는 그걸 `stopBattle` 에 담아 다음 `render()` 첫 줄에서 부른다. 정리 함수는 재생 위치 `{t, speed, running, tab, logf, wall, auto}` 를 돌려주고(`tab` = 나눔 배치 우측 열에서 고른 판 · `logf` = 로그를 거른 주체 · `wall` = 마지막으로 시각을 민 실제 시각 · `auto` = 결과 띠가 다음 런을 세던 중, 2026-09-11), 렌더러가 `state.battle.resume` 에 담아 다음 mount 의 `opts.resume` 으로 넘긴다 — 재렌더(가방 클릭 · 언어 전환)에도 재생이 이어진다 (2026-08-27)
- **되감기(`catchUp`)는 상태만 적용한다** (2026-09-24) — 카드 · 게이지 · 물약 칸 · 라운드 트랙 · 누적 판을 그리는 함수는 `state.catchUp` 이면 그냥 지나가고, 되감기가 끝나면 `paintCaughtUp` 이 한 번 그린다. 로그는 `logLine` · `logWide` 로만 적는다(되감는 동안은 재료만 모았다가 주체마다 마지막 `LOG_KEEP` 줄만 짓는다). **새 이벤트 처리에서 DOM 을 직접 만지면 이 가드를 같이 단다** — 안 달면 관전 중 클릭마다 그 런의 사건 수만큼 그 DOM 작업이 되풀이된다([DEV_PLAN.md §4](docs/client/DEV_PLAN.md) #64)
- 로그 목록 스크롤 · 누적 판 다시 그리기는 **걸음 끝에 한 번**(`step`)이다 — 줄마다 `scrollHeight` 를 읽으면 그때마다 레이아웃이 강제된다
...
- 배속 · 일시정지는 **진행 속도의 문제**지 결과의 문제가 아니다 — 같은 시드 · 같은 장비면 같은 결과다. **철수는 결과를 바꾼다** — 진행 중인 라운드를 버린다. 보상은 라운드를 이긴 순간 들어오고 그때마다 저장된다 ([ARCHITECTURE.md §5](docs/client/ARCHITECTURE.md))
