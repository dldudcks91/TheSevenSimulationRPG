...
**시간축이 활동을 가른다 — 실시간 = 전투(원정·의뢰) / 오프라인 = 비전투 전부.**

| | 접속 중 | 오프라인 |
|---|---|---|
| 활동 | 원정(실시간 전투) · **의뢰**(실시간 전투, 별도 채널) · 아이템 정리 | 파견 — 파견처 · 탐험 등 |
| 산출 | 장비 드롭 | 재료 · 재화 · 해금 |
...
2. ~~전투 창구는 하나 — 그래서 관전이 성립한다. 관전을 죽이는 제안(동시 원정 확대 등)은 이 조항과 먼저 대조한다.~~ — **2026-08-31 의뢰 신설로 개정**: 전투 채널이 **원정·의뢰 둘**일 수 있다. 각자 자기 탭(원정 탭 / 의뢰 탭)에서 실시간으로 돌고, **관전 대상은 유저가 지금 보고 있는 탭**이 정한다. **원정 파티와 의뢰 파티는 완전히 분리된 인원**이어야 한다 — 로스터를 겹쳐 쓰지 않는다(GAME_DESIGN.md §9).
3. **병렬 가동은 비전투가 전담** — 로스터 전원을 굴리는 압력(타겟 니즈 3)은 파견·탐험으로 푼다. 전투를 복제해서 **그 압력을 풀지는 않는다.** Lootun과 갈리는 지점은 **"전투가 하나냐"가 아니라 "관전이 가능하냐"** — Lootun은 3인 팀을 여러 개 동시에 돌려서(로스터 10~15) 병렬을 만들고 그 대가로 관전이 구조적으로 불가능하지만, 본작은 원정·의뢰가 동시에 돌아도 **파티가 완전히 분리**되고 **탭으로 관전이 유지**된다. 의뢰는 로스터 병렬가동 압력을 풀려고 만든 게 아니라 타겟 보스/던전 접근용이다 — 압력 해소는 여전히 파견·탐험의 몫이다.

미확정 (락 아님): 동시 원정 1의 영구 여부 · 파견 종류 어휘(탐험·여행·아르바이트 — 채집은 2026-08-31 신설로 해소, 수색은 선술집 하위 기능이라 파견이 아니다) · 기능 해금 사다리 — [GAME_DESIGN.md §10](docs/game_design/GAME_DESIGN.md)
