...
> **상태: 1단계 완료(R136 · 09-22) · 2단계 = 표준 모양 개편(R151 · 09-24 — 사용자 승인 「진행 · 탈출구 없음」) · 3~4단계 남음.** 이 문서가 결정과 순서의 SSOT이고, 단계가 끝나면 계약(INTERFACE)과 데이터 README가 SSOT를 넘겨받는다.
...
| # | 결정 | 왜 이게 확장성에 맞나 |
|---|---|---|
| **S3** | **스킬 대상은 skill.csv 에 하나** + **하는 일 줄마다 `target` 칸**(`-` = 스킬 대상 · 필요할 때만 `self` · `party` 등). **2단계에서 들어온다**(첫 사용자 = 결투). [개정 09-24 · 2단계 D2] 줄의 대상은 **줄마다 고른다** — 고르는 대상(아군 · 적에게 거는 창)은 결정론이라 같은 답이다. 굴림이 드는 **공격 대상을 한 번 굴려 여러 줄이 나눠 쓰는 것**은 첫 사용자(메테오 「때린다 + 화상」)와 같이 온다 | 메테오 「때린다 + 화상」이 **같은 적**에게 간다(줄마다 따로 고르면 다른 적에게 갈 수 있다). 결투 · 사자후 ③ · 동귀어진(나의 HP)처럼 줄마다 대상이 다른 스킬을 전용 코드 없이 적는다 |
...
| **S2-a** | **걸린 효과 행이 숫자(값 · 시간)를 든다**(S2 에서 보여 준 모양 그대로). 같은 효과를 다른 세기로 쓰면 행을 나눈다. [개정 09-24 · 2단계 D3] **창의 열쇠 = 걸린 효과 id** — ~~거는 스킬 id~~ 는 한 스킬이 한 유닛에 효과 둘을 걸면 뒤 줄이 앞 줄을 덮었다. 창은 건 스킬 id(`s`)를 따로 들어 이벤트의 `s` 는 그대로다. 같은 효과를 여러 스킬이 걸면 한 창을 갱신한다(지금은 효과마다 거는 스킬이 하나라 동작이 같다) — 「스킬마다 따로」가 필요한 효과가 오면 걸린 효과에 겹침 규칙 칸을 더한다(3단계) | 지금 규칙 「같은 효과 재시전 = 시간 갱신 · 다른 효과의 같은 능력치 = 덧셈」을 그대로 잇는다(골든 불변). 겹침은 효과가 정하는 것이 표준이다(WoW 오라 · Dota modifier) — 화상처럼 여러 스킬이 거는 효과는 하나로 갱신되는 편이 기본값에 맞는다 |
...
### 2단계 — 표준 모양으로 (결투 · 자폭을 데이터로) [개정 2026-09-24 · R151 · 사용자 승인 「진행 · 탈출구 없음」]

표준(WoW Spell → Effect → Aura · Dota Modifier · GAS attribute)에 스킬 「종류」 구조를 맞춘다. **화면 · 세이브 · rng 순서 불변 리팩터**이고, 동작은 **옛 창 열쇠 충돌(결투의 두 창이 같은 열쇠) 하나를 고친 몫만** 갈린다(아래 검증 결과). 스킬 id 전용 스크립트 탈출구는 **두지 않는다**(사용자 09-24) — 결투 · 자폭이 데이터로 빠지면 전용 코드를 쓰는 스킬이 0 이다.

| # | 결정 | 첫 사용자 |
|---|---|---|
| **D1** | **대상 표 하나** — `ATTACK_TARGETS`(누구를 · 몇 번 · 어떤 배율 — 함수 그대로) 옆에 **`PICK_TARGETS`**(고르기만 — 아군 넷 + 적에게 거는 창 둘 · `{side, pick}`)를 세우고 `skill_runtime.targetsOf` 의 switch 를 걷는다. `enemy_single` 은 공격 뜻 하나 · 적에게 거는 창의 「HP 최대」는 **`enemy_hp_max`**(S3-a · kni_duel 한 행). 공격 대상이 `decay` 를 무슨 뜻으로 쓰나는 **`HIT_DECAY`** 가 든다(skill.js `usesDecayOf` 하드코딩 제거). `SUPPORT_TARGETS` · `DEBUFF_TARGETS` 배열은 `PICK_TARGETS` 의 `side` 가 대신한다. **오오라는 이 표를 안 거친다** — `battle.applyAuras` 가 자기 또는 편 배열 전체에 걸어서, 로드가 오오라 대상을 `self` · `party` 로 좁힌다(검토 09-24) | 결투 |
| **D2** | **한 스킬 여러 줄** — 「줄 정확히 1개」 제약 해제 · `skill_effect.csv:target`(`-` = 스킬 대상 · S3) · 런타임이 줄을 `seq` 순으로 실행한다(`rt.cast` — 차례 시전 · 사건 시전이 같이 쓴다). 줄마다 적 목록을 다시 본다(앞 줄이 쓰러뜨린 적은 뒤 줄의 대상이 아니다 · 적이 없으면 `hit` · `fixed` 줄은 건너뛴다). 설명창 · 미리보기는 **첫 줄 그대로** — 줄마다 문장은 SCREEN_DESIGN §2 먼저(`/ui` · 아래 「남는 것」) | 결투 |
| **D3** | **창 열쇠 = 걸린 효과 id**(S2-a 개정) — 창은 건 스킬 id `s` · 라운드 규칙 `roundEnd` 를 함께 든다. `buff` · `buffEnd` 이벤트의 `s` 는 **그대로 스킬 id**다(재생기 칩 · 골든). `buff_absent` 는 「시전자에게 **이 스킬이 건** 창이 없다」(`s`)로 본다. ⚠ 이벤트가 창 열쇠를 안 실어 **재생기 칩은 `s` 로 합쳐진다** — 한 유닛에 같은 스킬의 창이 둘이면 칩이 하나다(부채 #68) | 결투 |
| **D4** | **`skill_status.csv:round_end`**(`keep` · `close`) — `beginRound` 가 **파티**의 `close` 창을 닫는다(적의 창은 적 배열과 함께 사라진다 · 자리 · 순서 · 이벤트 모양 불변). 결투 전용 닫기 제거. 오오라 효과는 `keep` 이어야 한다(로드 검사) | 결투 |
| **D5** | **결투 = 줄 둘** — ① `apply` 지목 `kni_duel`(스킬 대상 `enemy_hp_max` · `keep`) ② `apply` 받는 피해 감소 `kni_duel_guard`(`self` · `close`). `castBuff` 의 결투 분기 제거. 지목 창의 `value`(0.2)는 `buff` 이벤트 `v` · 설명창 문장이 읽어 **그대로 둔다** — ⚠ 피해 감소 값이 두 행에 선다(줄마다 문장이 오면 둘째 줄이 제 값을 말한다) · ⚠ 몬스터가 영웅에게 건 지목은 라운드를 넘어 남는다(`keep` — 지금 동작 그대로 · 부채로 적는다) | — |
| **D6** | **나가는 방식 표가 규칙을 든다** — `CASTS` = `{turn, aura, event}` 각 `{picked, cooled}`(차례에 고르나 · 쿨이 양수인가) · **사건 표 `EVENT_TRIGGERS`**(`on_death` — `once`: 한 유닛 한 번 · 되살아나도 안 풀린다). `fixed` 가 `run` 을 갖는다(`rt.dealIndirect` — 비직격 피해의 이음매는 battle.js 가 준다) · **`battle.blast` 제거** — `downed` 끝에서 `rt.fire(u, 'on_death', t)` → `rt.cast`. **「늘 거짓 조건」 자물쇠 제거** — `castable` 이 `CASTS[cast].picked` 를 먼저 본다 · `cast_condition` 어휘 = `CONDITIONS`(차례) ∪ `EVENT_TRIGGERS`(사건) | 자폭 |
| **D7** | **능력치 창은 능력치(attribute)마다 모아 계산** — `EFFECTS` 는 「무엇을 미나」만 선언하고 `refreshDerived` 는 **능력치 표**(`ATTRS`)를 순서대로 돈다: 데미지 괄호 · 주기 · **방어 = guard + def_pct** · **저항 = guard + 원소 창** · 최대 HP · 재생 · 받는 피해 감소. 옛 「`EFFECTS` 키 순서가 계약」(def_pct 가 guard 뒤라야 둘을 다 들었다) 제거 — 식은 부동소수 연산 순서까지 같다 | — |
| **D8** | **표식은 선언 + 조회 둘** — `taunt` · `duel` · `onhit_element` · `attack_splash` 를 `EFFECTS` 에 `state: true` 로 적고, 읽는 쪽(battle `pickTarget` · runtime `basicAttack`)은 `stateOf(u, stat)`(첫 창) · `sumOf(u, stat)`(창 합)만 쓴다 — `b.stat === …` 직접 훑기 제거 | — |

**2단계가 하지 않는 것**(첫 사용자와 같이 — 「쓰는 날 추가」): 틱 · 스택 · 끝날 때 실행 · 반응(훅 소비자) — 3단계 · 상태이상 · 적 자리 — 4단계 · 노드 · **줄 사이 공격 대상 공유**(S3) · **줄마다 설명 문장**(`previewOf` 의 `rows` · SCREEN_DESIGN §2 먼저 — 결투가 두 문장이 되고 `round_end = close` 창은 「라운드가 끝날 때까지」로 말한다) · `strikeOnce` 의 「타격 동안만」 임시 필드 정리.

**검증** — 착수 전 `src` 사본(스크래치)과 작업본을 따로 띄워 같은 대조 페이지를 찍는다. ① 골든 50런(`parties` · `runs`) ② 직업 킷 전부 × 2 변형(그대로 · 강한 파티) × 전 스테이지 × 시드 3 = 1470판의 **결과 전체 해시** ③ 원정 중 갈아입기 70판 ④ 설명창 HTML 156벌 · 미리보기 78벌 — **넷 다 한 글자도 안 바뀌어야** 통과 ⑤ `test.html` 실패 목록이 기준 벌과 같다(새 실패 0) ⑥ `index.html?dev=battle` 렌더(로그인 게이트는 import-map 우회).

**검증 결과 [2026-09-24]** — ④ 설명창 · 미리보기 · ① `parties` 는 한 글자도 안 바뀌었다. ①②③ 은 **기사 킷의 판 몇 개만** 갈렸다(골든 1런 `tl` · 킷 12판 · 갈아입기 2판) — 원인은 **옛 창 열쇠 충돌**이다: 옛 판은 결투의 지목 창과 시전자 창을 같은 열쇠(`kni_duel`)로 들어, 결투를 쓴 기사가 상대 결투에 지목당하면 나중 창이 먼저 창을 지웠다. D3 가 고치려던 그 결함이다. **증명** — 옛 코드에 「시전자 창만 다른 열쇠」 하나를 얹은 세 번째 벌과 새 코드가 ①②③④ 전부에서 **한 글자도 안 다르다**. ⑤ 기준 `FAIL 415/418` · 작업 `FAIL 419/422`(단정 +4) — 실패 셋이 글자까지 같다(설명창 Alt 각주 · 건설 문턱 · 골든 50/50 — 이 개편 전부터). 새 단정 6개는 옛 코드에 돌리면 전부 빨간불이다(헛돌지 않는다). ⑥ ko · en 렌더 정상. 골든 재촬영은 대기(INTERFACE §5-5 「R151 재촬영 대기」).
...
- 「첫 줄만 읽는 곳」 — 2단계가 줄 1개 제약을 풀었고 **전투는 줄 목록**이 됐다. 설명창 · 미리보기(`previewOf` · `ui/tip.js:skillLines`) · 재생기 칩의 원소(`ui/battle.js`)는 **아직 첫 줄**이다 — 지금 여러 줄은 결투 하나라 틀린 값이 안 나온다. **줄마다 문장 작업**(SCREEN_DESIGN §2 먼저 · `/ui` · 부채 #63 · #68) 때 그 자리들을 전부 줄 목록으로 바꾼다(grep `effects[0]`)
