...
gamble.js(표 넷, balance) ───────────────┤  도박장 슬롯 — 심볼 · 코인 · 라인 · 판돈 표를 검증하고 **한 판을 굴려 결과를 데이터로 낸다** · 무상태 · 세이브를 모른다 (§2-15 · 2026-09-24)
commission.js(표 넷, balance) ──────────┤  의뢰 — 틀 · 종류 · 등급 · 종족 표를 검증하고 **카드 한 장을 굴리고 처치 · 드롭을 대조한다** · 무상태 · 세이브를 모른다 (§2-16 · 2026-09-24)
state.js(deps: hero, item, battle, skill, construction, gamble, commission, balance, …) ──┘
```
...
`createFormula(balance) → { roundPct, pctOption, growthMult, upgradeMult, weaponDamage, armorDefense, mitigation, physicalDefense, resCap, appliedResist, reductionMult, hitChance, strike, indirect, leech, effectiveCd }`
...
strike(rng, a, d):
  ...
  crit = rng() < (a.crit ?? 0);  crit 이면 v ×= critDmg ?? 1                    ← rng ③  치명 확률 상한 없음 (2026-09-25)
...
- **추가 피해는 치명과 따로 굴려 겹친다** — 둘 다 터지면 곱이다. **직격에만** 붙는다 — `indirect`(반사·도트)는 받지 않는다 [2026-09-10 · R72]
...
| `masteryNodes` | `[mastery_node.csv 행]` — 노드별 `name_kr/name_en` 은 표시 이름, `stat` 은 효과 채널이다. 랭크당 값·상한·해금 레벨은 **키 이름만** 들고 `balance` 에서 읽는다 | mastery_node.csv |
...
| `crit_rate` · `crit_damage` | `base_crit_pct` + Σ 접사 — 상한 없음(2026-09-25) / `1 + (base_crit_damage_pct + Σ 접사 − 1) × statCoef(luck)` 소수 4자리 — 보너스 몫에만 운 계수(2026-09-25 C안 · 몬스터도 같다) |
...
| `gold_find` · `item_find` | `roundPct(Σ접사 × attrMult(luck))`(1% 단위 · R111) — **곱이라 접사가 0이면 0**. 운의 **선형** 계수(`attrMult`)는 이 둘에만 걸린다 — 치명타 피해는 복리 `statCoef(luck)`(위 행) |
...
| `buffs` | `{statusId: {stat, v, until, element, by, s, roundEnd}}` — **창 하나 = 걸린 효과 하나** [개정 2026-09-24 · R151 · PLAN_skill_structure 2단계 D3 — ~~`{skillId: …}` 창 하나 = 스킬 하나~~ 는 한 스킬이 한 유닛에 효과 둘을 걸면 뒤 줄이 앞 줄을 덮었다]. **중첩 없음**, 같은 효과 재시전은 `until` 갱신 · 다른 효과의 같은 `stat` 은 덧셈. `s` = **건 스킬 id** — `buff` · `buffEnd` 이벤트의 `s` 가 이것이다(재생기 칩 · 골든 — 지금은 효과 id 와 같은 행이 대부분이라 이벤트가 안 바뀐다). ⚠ 이벤트는 창 열쇠를 안 싣는다 — **한 유닛에 같은 스킬의 창이 둘**이면(결투를 쓴 기사가 상대 결투에 지목당함) 재생기 칩은 `s` 하나로 합쳐지고 먼저 닫힌 창의 `buffEnd` 가 칩을 걷는다(부채 #68) · `roundEnd` = 걸린 효과의 `round_end`(`keep`/`close` — 아래 「라운드 경계」) · `element`(평타 부여가 때릴 원소) · `by`(건 자의 key — 지목이 읽는다)는 2026-09-09 신설. **`until: Infinity` 는 오오라**(만료가 영원히 안 걸린다 · 이벤트에서는 `until: null` · R98) · `v` 가 **음수면 디버프**(적에게 건 창) · **`quiet: true` = 무기 옵션 창**(키 `wx:…` — 열 때도 닫을 때도 이벤트를 안 낸다 · `s` 가 없다 · 2026-09-11 R78). 창을 **손으로 만든 곳**(`s` 가 없는 창)은 열쇠를 `s` 로 읽는다 |
| `fired` | **사건 스킬을 쏜 기록** `{skillId: true}` [신설 2026-09-24 · R151 — ~~`blown`~~(자폭 전용 표식) 대체] — `EVENT_TRIGGERS[사건].once` 인 사건(`on_death`)은 **한 유닛 한 번**이다 · 되살아나도(불러내기) 안 풀린다(보상 `rewarded` 와 같은 규칙) · 갈아입기가 안 건드린다 |
...
**result** — `{won, reason, durationSec, party:[{key, uid, hpMax, period, atkMin, atkMax, matkMin, matkMax, atkType, stats, actives:[skillId], ready:[초]}], timeline:[ev], xpTotal, gold, kills:{monsterId:n}, killGrades:{monsterId:{grade:n}}, drops:[item(uid null)], downed:[heroUid], roundsCleared, rounds:[{n, kind, killed:[monsterId], eliteSin}], strikes:{party:{n,miss}, enemy:{n,miss}}, contrib:[{uid, dealt, taken, kills}], casts:{skillId:n}, potion:{max, slots:[{id, heal} | null], used}}`
...
- `killGrades` = `kills` 를 **처치 순간의 등급**(`normal` · `elite` · `stage_boss` · `chapter_boss`)으로 가른 것 [신설 2026-09-24 · R153 — 의뢰의 「정예 · 보스 n마리」] — 몬스터마다 등급별 합이 `kills` 와 같다. 라운드 요약(`advance` 가 내는 `s`)에도 같은 이름으로 든다 · 세는 것뿐이라 **rng 0 · 타임라인 밖**(골든 지문 무관)
- **`xpTotal` · `gold` · `kills` · `killGrades` · `drops` 는 이긴 라운드의 몫만 센다** [개정 2026-09-14 · R89] — 진 라운드(전멸 · 시간 초과)에서 잡은 몬스터는 `rounds[].killed` · `contrib` 에만 남는다(사실의 기록). 드롭 **판정 굴림은 처치 순간 그대로** 돌아 rng 순서가 안 바뀐다 — 라운드 몫으로 모아 두었다가 이기면 결과에 넣는다
...
| `blast` | `a, d, s, dmg, dhp` | **자폭** [신설 2026-09-21 · battle_design §9-6 · skill_design §12-9] — 비직격 **고정 피해**(하는 일 `fixed` 의 이벤트 · 지금 쓰는 스킬은 자폭 하나). `a` = 터진 쪽(쓰러진 유닛) · `d` = 맞은 쪽 · `s` = 그 스킬(`mon_selfdestruct`) · `dhp` = 맞은 쪽의 남은 HP. **`down` 바로 뒤**에 서고 대상 수만큼 잇따른다(광역 `hit` 과 같은 모양) · **rng 0** |
| `down` | `u` | 전투불능 |
...
| `buffEnd` | `u, s` (+ **`hpMax`·`dhp`**) | 창 만료 (그 틱의 행동 처리 **앞에서**) · **`round_end = close` 창**(지금 결투의 시전자 피해 감소)이 라운드 경계에서 닫힐 때(그 라운드의 `round` 이벤트 바로 앞 · 2026-09-10 · 일반 규칙 2026-09-24 R151) · `s` = 그 창을 건 스킬 id(창의 `s`) · 갈아입기로 사라진 오오라(다음 `round` 바로 뒤 — 다시 건 오오라의 `buff` 보다 앞 · R98). 닫히며 최대 HP 가 바뀌면 **그 틱의 마지막 `buffEnd`** 가 `hpMax`·`dhp` 를 싣는다 [신설 2026-09-21 · 부채 #50] — 한 틱에 여러 창이 닫혀도 실린 값은 **전부 닫힌 뒤**의 상태다 |
| `end` | `won, reason` | **마지막 이벤트**. `reason ∈ clear \| wipe \| timeout` (`retreat` 폐기 2026-09-03) |

유닛 키: 파티 `p0..`, 적 `e0..`(라운드마다 0부터).

**순서 보장** — ① `t` 는 단조 비감소 ② `round` 가 라운드의 첫 이벤트(그 앞에 붙는 것은 경계의 `refit` · `round_end = close` 창의 `buffEnd` 뿐 — 오오라 창의 `buffEnd` · `buff` 는 `round` **뒤**다 · R98 · **라운드 도중의 `refit` 은 그 시각에 서고 곧바로 그 갈아입기의 오오라 `buffEnd` · `buff` 가 잇는다** — R130) ③ `end` 가 마지막 ④ 같은 `t` 안에서는 배열 순서가 곧 발생 순서(스킬 이벤트도 같다 — `skill` 뒤에 그 시전의 타격·회복·버프가 이어진다).
...
| 표 셋 분리 [2026-09-22 · R136 · PLAN_skill_structure 1단계] | 스킬 정의가 표 셋(스킬 · 하는 일 · 걸린 효과 — §2-8)으로 갈라졌어도 **타임라인 이벤트는 하나도 안 바뀐다** — `skill` · `buff`(`s` = 거는 스킬 id · `stat` = 걸린 효과의 능력치) · `buffEnd` · `heal` · `summon` · `call` · `blast` 의 이름 · 필드 · 순서 · rng 소비가 그대로다(골든이 잠근다). **표준 모양 개편(2026-09-24 · R151 · PLAN_skill_structure 2단계)도 같다** — 한 스킬 여러 줄 · 창 열쇠 = 걸린 효과 id · 라운드 경계 규칙 · 결투 줄 둘 · 사건 스킬 일반 경로를 넣고도 이벤트 모양 · rng 소비 순서는 그대로다. **갈린 것은 하나 — 옛 창 열쇠 충돌을 고친 몫**이다: 옛 판은 결투의 지목 창과 시전자 창을 **같은 열쇠(`kni_duel`)** 로 들어, 자기도 결투를 쓴 기사가 상대 결투에 지목당하면 나중 창이 먼저 창을 지웠다(시전자의 피해 감소가 사라지거나 받은 지목이 사라졌다). 증명 — 옛 코드에 「시전자 창만 다른 열쇠」 하나를 얹은 판과 새 코드가 골든 50런 · 직업 킷 전부 × 전 스테이지 × 시드 3(1470판) · 갈아입기 70판 · 설명창 156벌에서 한 글자도 안 다르다 |
| 여러 줄 [2026-09-24 · R151] | 시전 한 번 = **하는 일 줄을 `seq` 순으로** 실행한다(`skill_runtime.cast` — 차례 시전 · 사건 시전이 같이 쓴다). 줄마다 **그 줄의 대상**(`skill_effect.csv:target` · `-` = 스킬 대상)을 고르고 **적 목록을 다시 본다** — 앞 줄이 쓰러뜨린 적은 뒤 줄의 대상이 아니고, 적이 하나도 없으면 `hit` · `fixed` 줄은 건너뛴다(굴림도 없다). 굴림은 줄 순서대로 그 줄의 것을 쓴다 — 한 줄 스킬은 종전과 같다. ⚠ 공격 대상을 한 번 굴려 여러 줄이 나눠 쓰는 것은 아직 없다(첫 사용자와 같이 · PLAN S3) |
...
| 자폭 [2026-09-21 · 사용자 확정 · battle_design §9-6 · skill_design §12-9] | **쓰러지는 순간 적 전원에게 한 번 터진다.** 자리 = `battle.js` 의 `downed` **끝**(`down` 이벤트 · 처치 정산 · `down` 훅이 다 끝난 뒤)에서 **`rt.fire(u, 'on_death', t)`** — 그 유닛의 칸에서 `cast=event` 이고 `cast_condition=on_death` 인 스킬을 **칸 순서대로** 일반 시전 경로(`rt.cast`)로 쏜다 — 차례가 아니라서 `skill` 이벤트 · 시전 수(`casts`) · `cast` 훅 · 쿨을 **안 남긴다**(종전 그대로) [2026-09-24 · R151 — ~~`battle.js:blast` 전용 함수~~]. `on_death` 는 `once` 사건이라 **`fired` 에 그 스킬이 서 있으면 안 쏜다**. <br>**세기 = `(atkMin + atkMax) / 2 × mult_pct × statMult`**(그 스킬의 `fixed` 줄 · §2-8 · 능력치 계수는 데미지 슬롯이 없으면 1 — 설명창 `previewOf` 와 같은 식) — 범위를 **굴리지 않고 중앙값**을 쓴다(rng 0 — 자폭이 없는 판의 수열은 종전과 완전히 같다). `formula.indirect` 를 지나 **방어 · 저항 · 피해 감소 · 배리어를 하나도 안 빼고** 맞는 쪽 HP 를 직접 깎는다(고정 피해). 적중 게이트 · 치명 · 스킬 배율 · 조건부 % · 추가 피해 없음 · **흡혈 · 반사 · 경직 · 타격 훅을 유발하지 않는다**(§9-6). <br>대상 = 그 줄의 대상(`enemy_all` — 살아 있는 적 전원 · 배열 순) · 맞히는 일은 battle.js 가 주는 이음매 `dealIndirect(a, d, dmg, s)` 가 한다(HP 차감 · `blast` 이벤트 · 기여 · 전투불능). 기여는 양쪽 다 센다(터진 쪽 `dealt` · 맞은 쪽 `taken`) · 맞아서 쓰러지면 그 자리에서 `downed` 가 다시 돈다. **한 마리당 한 번** — `fired` 는 되살아나도 안 풀린다(보상 `rewarded` 와 같은 규칙). 지금 구조에서 연쇄는 없다(몬스터의 자폭은 파티만 때린다) |
...
| 추가 피해 | `proc_chance_pct > 0` 인 스킬의 **직격마다** `strike` 가 치명 **뒤에** 1회 더 굴린다(`rng() < min(확률, 1)`) — 터지면 `× proc_mult_pct`(치명과 곱). 빗나감은 굴리지 않고 **반사·기본 공격·평타 부여 추가타는 싣지 않는다**. 터진 타격만 `hit` 이벤트에 `proc: true` · 훅 `hit`/`hitTaken` payload 에 `proc` [2026-09-10 · R72 · battle_design §9-2] |
...
| `duel` | **결투는 줄 둘이다** [2026-09-24 · R151 — ~~`castBuff` 안의 결투 분기가 시전자 창을 따로 열었다~~] — ① 지목 창(걸린 효과 `kni_duel` · `stat = duel` · 스킬 대상 `enemy_hp_max` — 생존 적 중 HP 최대 · rng 0) ② **시전자 자신의 `dr_pct` 창**(걸린 효과 `kni_duel_guard` · 줄 대상 `self` · `v` = 기사 자신이 받는 피해 감소 %). 둘 다 같은 `until` 이고 `buff` 이벤트가 이 순서로 둘 난다(`s` 는 둘 다 `kni_duel`). `stat = duel` 창은 **표식**이다(소비자 `battle.pickTarget`). ② 는 `round_end = close` — 아래 「라운드 경계」 행 |
| 라운드 경계 [2026-09-24 · R151 · 일반 규칙] | **`round_end = close` 인 창은 라운드가 바뀌면 닫힌다** — 자리 = `beginRound` 첫머리(소환물을 걷은 뒤 · 스폰 굴림 앞). **파티만 본다** — 적의 창은 적 배열과 함께 사라진다. 파티 순 · 창 순으로 닫고 **기존 `buffEnd`** 를 낸다(만료와 같은 모양 · 그 라운드의 `round` 이벤트보다 앞 · 최대 HP 가 바뀌면 그 유닛의 마지막 `buffEnd` 가 `hpMax` · `dhp` 를 싣는다) · rng 0. 쓰는 효과는 결투의 시전자 창(`kni_duel_guard`) 하나다(창이 999초라 만료로는 안 닫힌다 · 2026-09-10 R72 후속의 규칙을 데이터로 옮겼다). ⚠ 지목 창(`kni_duel`)은 `keep` 이다 — 영웅이 건 지목은 적과 함께 사라지지만 **몬스터가 영웅에게 건 지목은 라운드를 넘어 남는다**(동작 불변 리팩터라 안 바꿨다 · DEV_PLAN §4) |
...
`export const SAVE_VERSION = 38`  [v38 = **다부대** — `run`(단수) → `runs`(부대마다) · `reports[].preset` · `notice.runs` · 한 영웅은 한 편성에만 · 2026-09-23] **[v38 에서 또 끊었다 — 그 전 세이브는 열지 않는다 · §4 · 2026-09-23]** · ~~v37 = 건설(`buildings` · `research` · `progress.peakTotal` · R137 · 2026-09-22)~~

`createGameSystem(deps)` — `deps`: `hero, item, battle, skill, tactic, construction(§2-14 · R137), gamble(§2-15 · R149), commission(§2-16 · R153), balance, equipSlots [{id, part}](착용 위치 8), stages(byId), stageOrder [id], monsters(byId), codex {levels:[cards_to_next], bonus:[%], statByNum:{stage_num: statKey}}`, **`sins [죄종 id]`** · **`searchStories`**(= `search_story.csv` 파싱 행) [신설 2026-09-09 — 수색]. **`makeRecipes`** `{part: {ore, timber, dust}}`(= `make_recipe.csv`) · **`mineNodes`** · **`logNodes`**(= `mine_node.csv` · `log_node.csv` 를 tier 순으로 편 행 `{id, tier, yieldId, …}`) [신설 2026-09-15 — 제작 · R96]. 레시피는 생성 때 검증한다 — **광석 · 목재 · 가루가 모두 1 이상**이 아니거나 없는 부위면 throw (보완재 · 원정 쪽 입력 — item_design §5-1 · §7-1). **`potions`** `[{id, kind, tier, ko, en, heal, craftGold, startOwned}]`(= `potion.csv` 행 순서 · ~~`craftable`~~ 2026-09-22 삭제 — 단계는 제련소 랭크가 연다 · R137) [신설 2026-09-15 — 물약 · R103]. 이 표도 생성 때 검증한다 — `id` 유일 · `kind` 는 `heal` 하나(모르는 종류는 멈춘다) · 같은 종류 안에서 `tier` 는 1 부터 연속 · `heal` 은 단계마다 커진다 · `craftGold ≥ 0` · **`startOwned` = 시작 개수**(0 이상 정수 — 2026-09-21 · R124 · 전엔 0/1 플래그) · 이름 ko/en 이 비지 않는다 — 어기면 throw. **`balance.party_preset_count`**(편성 수의 **건설 전** 기본값)는 0 이상 정수이고 **시작 랭크의 편성 수**(`limitsOf(null).presets` — 기본값 + 처음부터 지어진 랭크의 더하기)가 1 이상이어야 한다 — 아니면 throw [2026-09-21 · R122 · 원정 랭크가 편성을 연다 2026-09-25 · R156 · 첫 편성은 기본값 1 — 원정 r1 은 장만 연다 · R161]. **`openAll`** `() → bool`(선택 · 없으면 늘 꺼짐) [신설 2026-09-24 · **개발 장치** — SCREEN_DESIGN §10-3 관리자 모드] — 켜져 있으면 「무엇이 열렸나」를 세는 셋(`limitsOf` · `hasFeature` · `constructionState.tabs`)이 **모든 건물을 최대 랭크로 친다** · **`stageUnlocked` 는 표의 모든 스테이지에 참**이다(장 잠금 · 직전 클리어를 안 따진다 — 그래서 `frontierChapter` 가 마지막 장이 된다 · 2026-09-24). 세이브의 `buildings` · `progress.cleared` 는 안 바뀌고(`stageLevelState` 의 상한은 클리어 기록 그대로), 짓기 쪽(`constructionState` 의 랭크 · `next` · `ranks` · `construct`)은 **실제 랭크**를 본다. 켜고 끄는 사이에 상태를 맞추지 않는다 — 칸 수를 읽는 자리가 읽을 때 상한에 맞춘다(`padSlots` · 불러오기).
...
| `toggleParty(state, uid, now)` | `→ {ok}` / `{ok:false, err}` | **고른 편성**의 파티에 넣고 뺀다 [편성 2026-09-21 · R122 — 번호를 안 받는다: 편성 탭은 늘 고른 편성을 편다 · `selectPreset`] · ~~**한 영웅이 여러 편성에 들어가도 된다**~~ → **한 영웅은 한 편성에만 든다** [개정 2026-09-23 · 다부대 · GAME_DESIGN §1-1] — 넣을 때 **다른 편성에 들어 있으면 거기서 뺀다**(거절이 아니라 이동 · 그 편성의 진형에서도 빠진다). 편성이 곧 부대라 겹치면 한 사람이 두 부대에 선다 · **원정 중에도 넣고 뺀다** — 도는 원정은 나간 인원 그대로 싸운다(`runParty` · R92 · 옮겨 둔 영웅 때문에 그 편성이 못 나가는 것은 `canDepart` 의 `busy`). err: `missing` · `full` · **`searching`**(수색 나가 있다 — 마을에 없으므로 편성할 수 없다 · 신설 2026-09-09). 넣을 때만 본다 — 뺄 때는 검사가 없다(나가 있는 영웅은 애초에 파티에 못 들어간다). (~~`injured`~~ 는 2026-09-03 에 검사 자체가 사라졌다 — 계약 문서에만 남아 있던 것을 09-06 에 지웠다) |
| `formationState(state, no?)` | `→ {tpl, caps, ranks, byUid, templates, shapes}` | **읽기 전용 · 순수하다** — **고른 편성**의 진형(§4 `presets[].formation` · 2026-09-21) · **`no` 를 주면 그 편성**의 것이다 [2026-09-24 · 다부대 — 안 보는 부대의 관전 자리를 그 부대의 진형으로 찍는다 · 없는 번호면 고른 편성]. `caps` = 랭크별 정원 · `ranks[r]` = 그 랭크의 uid 배열 · `byUid[uid]` = 랭크 번호 · `templates` = **표의 행 순서**(⚠ `Object.keys` 는 `'3'` 같은 정수형 키를 앞으로 끌어올려 첫 행을 못 준다) · `shapes[tpl]` = 그 템플릿의 정원. 신설 2026-09-09 |
...
| **`presetState(state)`** | `→ {count, activeNo, runNos, max, slotMax, presets:[{no, party:[uid], formation:{tpl, ranks}, potionSlots:[{id, short} \| null], running, err}]}` | **편성 화면 상태** [신설 2026-09-21 · R122 · R124 · SCREEN_DESIGN §15 · §4-1 · **다부대 2026-09-23**] — **판정을 여기서 다 낸다.** `count` = 편성 수(`limitsOf(state).presets` — 원정 랭크가 연다 · R156) · `activeNo` = 고른 편성 번호(1 부터) · **`runNos`** = **도는 부대의 편성 번호 목록**(오름차순 · 없으면 `[]`) — ~~`runNo`(하나)~~ 대체 · **`max`** = 동시에 낼 수 있는 부대 수(`limitsOf(state).expeditions`) · `slotMax` = 물약 칸 수. 편성마다 **`running`** = 그 편성이 지금 도는 중인가(고르개의 「원정 중」 표시). 편성마다: `party` = 넣은 순서(`[0]` 이 리더) · `formation` = 저장된 진형(파생은 `formationState`) · `potionSlots` = 칸 수 길이 — 칸마다 `null`(빈 칸) 또는 `{id, short}` · **`short` = 재고가 모자라 런에서 빈 채 시작할 칸**(앞 칸부터 재고를 떼어 세므로 같은 물약이 여러 칸이면 **뒤 칸부터** 모자란다 · 표에 없는 id 도 `short` — `departRun` 이 칸을 채우는 것과 **같은 함수**다) · `err` = 그 편성으로 지금 나가면 나올 거절(스테이지 무관 — `noParty` → `searching` → **`busy`** → **`full`** 순 · 2026-09-23 다부대) 또는 `null`. 상태를 안 바꾼다 · rng 0 |
...
| `stageUnlocked(state, stageId)` | `→ bool` | **그 장이 열렸고**(`chapterOpen`) 첫 스테이지이거나 직전 스테이지(순서 기준)를 클리어했다 — 보스를 깨도 다음 장은 원정 랭크가 열어야 들어간다 [2026-09-24 · R152 · construction_draft §2] · **`openAll` 이 켜져 있으면 `stageOrder` 의 모든 스테이지에 참**(개발 장치 · 2026-09-24 · R155) |
| **`chapterOpen(state, chapter)`** | `→ bool` | **그 장이 원정 랭크로 열렸나** — `chapter ≤ limitsOf(state).chapters` [신설 2026-09-24 · R152]. 잠긴 스테이지 행이 「원정 n랭크 필요」(`needOf('chapters', n)`)와 「이전 스테이지 클리어 필요」 중 무엇을 말할지 가른다 · rng 0 |
| `stageLevelState(state, stageId)` | `→ {base, max, level}` / `null` | **스테이지 레벨** [신설 2026-09-14 · R87 · base_expedition_design §1-4] — **판정을 여기서 다 낸다.** `base` = `stage.csv:dlvl` · `max` = **클리어한 스테이지의 기본 레벨 중 최고**(키가 없다 — `progress.cleared` 에서 파생 · 아무것도 안 깼거나 그 스테이지가 더 높으면 `base`) · `level` = `base + progress.levelUp[stageId]` 를 `[base, max]` 로 자른 값. 올린 스테이지는 상한 이하라 **그걸 깨도 상한이 안 오른다** — 그래서 기본 레벨만 봐도 「클리어한 최고 레벨」이다. **처음부터 열려 있다**(2026-09-24 · R152 — 사용자 09-14 로 되돌림 · ~~원정 건물 랭크가 연다 — `open` 칸 · R137~~). 없는 스테이지면 `null` · rng 를 안 쓴다 |
| `setStageLevel(state, stageId, level)` | `→ {ok, level}` / `{ok:false, err}` | err: `missing`(없는 스테이지) · ~~`unbuilt`~~(2026-09-24 · R152 — 처음부터 열림) · `range`(정수가 아니거나 `[base, max]` 밖). **올린 양을 적는다** — 기본 레벨로 돌리면 기록을 지운다(안 올린 스테이지는 적지 않는다). 비용 없음 · 되돌리기 자유 · rng 를 안 쓴다. 신설 2026-09-14 · R87 |
| `canDepart(state, stageId, now, no?)` | `→ null` / `locked` / `missing` / `noParty` / `searching` / **`busy`** / **`full`** | 편성 `no`(기본 **고른 편성**)로 나갈 수 있나 — 이 순서로 본다 [편성 2026-09-21 · R122 · **다부대 2026-09-23**]: ~~원정이 안 열렸다(`expedition` · `unbuilt`)~~(2026-09-25 · R161 — 원정은 건물 밖 · 처음부터 열림) → 스테이지 잠김 → 없는 편성 → **그 편성의 파티가 비었다** → **그 편성에 수색 나간 영웅이 있다**(편성은 계획이라 든 채로 수색을 보낼 수 있고 여기서 막는다) → **`busy` = 그 편성의 영웅이 다른 부대에서 싸우는 중이다**(도는 부대의 인원은 출발 때 굳은 스냅샷이라, 그 영웅을 다른 편성으로 옮겨 두면 한 사람이 두 부대에 서는 셈이 된다 — 여기서 막는다) → **`full` = 도는 부대가 이미 `limitsOf(state).expeditions` 개다**(**그 편성이 이미 도는 중이면 안 센다** — 다시 보내는 것은 그 부대를 끊고 여는 것이라 수가 안 는다). **그 편성의 원정이 도는 중이어도 막지 않는다** — 보내면 `departRun` 이 **그 부대만** 끊는다 [R92] · ~~`injured`~~ 없음 (2026-09-03) |
| `runParty(state, no?)` | `→ uid[]` | **지금 싸우는 영웅** [신설 2026-09-14 · R92 · **다부대 2026-09-23**] — `no` 를 주면 **그 부대**가 나갈 때의 인원, 안 주면 **도는 모든 부대의 인원을 합친 것**(중복 없음 · 편성 번호 순). 판정이 아직 없는 그 부대 리포트의 `party` 다 · 도는 부대가 없으면 `[]`. 원정 중에도 편성을 바꾸고 고르므로 **그 편성의 지금 파티와 다를 수 있다**. `dismiss`(`running`) · `searchSend`(`party`) · `searchState.ready` 가 **인자 없이** 불러 막고(어느 부대든 싸우는 중이면 막는다), 영웅 띠의 「원정 중」이 이것을 읽는다 · rng 를 안 쓴다 |
| **`runOf(state, uid)`** | `→ 편성 번호` / `null` | **그 영웅이 싸우는 부대** [신설 2026-09-23 · 다부대] — 도는 부대 중 그 영웅이 든 부대의 편성 번호 · 없으면 `null`. `heroBusy` 가 `'run'` 이라고만 말하는 자리에서 **어느 부대인지**가 필요할 때 읽는다(영웅 띠의 「1부대 원정 중」 · `canDepart` 의 `busy`) · 한 영웅은 한 부대에만 서므로 답은 하나다 · rng 0 · 아무것도 안 바꾼다 |
| `heroBusy(state, uid)` | `→ 'run' \| 'search' \| null` | **영웅이 지금 하는 일** [신설 2026-09-22 · 구조 감사 · **다부대 2026-09-23**] — `'run'` = **도는 어느 부대든** 그 인원(`runParty(state)` — 부대를 안 가린다 · 어느 부대인지는 `runOf`) · `'search'` = 수색 나감 · `null` = 마을. **영웅을 붙잡는 활동의 판정은 이 한 곳이다** — 편성(`toggleParty` 의 `searching`) · 출발(`canDepart` 의 `searching`) · 해고(`dismissState`) · 수색(`searchSend` 의 `party` · `searchState.ready`) · 영웅 띠의 「지금 하는 일」이 모두 이것을 읽는다. 파견 · 훈련처럼 영웅을 붙잡는 활동이 생기면 여기에 더한다. 둘은 겹치지 않는다(싸우는 영웅은 수색에 못 나가고 수색 나간 영웅이 든 편성은 못 나간다). **쓰러짐(`run.fallen`)은 하는 일이 아니라 전투 안의 상태**라 여기 없다 · rng 0 · 아무것도 안 바꾼다 |
| **`constructionState(state)`** | `→ {built, total, tabs, buildings: [{id, name, tab, rank, maxRank, next, ranks}]}` | **건설 탭이 읽는 한 벌** [신설 2026-09-22 · R137 · construction_draft §11] — 건물마다 지금 랭크 · **다음 랭크 판정**(`construction.nextState` — 조건 · 비용(가진 양 포함) · 여는 것(준비 중 표시) · err) · **랭크마다 한 줄** `ranks: [{rank, built, effects, require}]`(조건은 지금 값 포함 — 비용은 다음 랭크의 `next` 만 든다 · **더하기의 `total`** = 그 건물을 그 랭크까지 지었을 때의 합(켜기가 심는 첫 단계 포함) — 「챕터 n 해금」 같은 누적 문장이 읽는다 · 2026-09-24 · R152 · 문구 R161) · `tabs` = 화면 탭마다 열렸나(`construction.tabs` — 흐린 탭 · **`openAll` 이 켜져 있으면 전부 열림**) · `built` / `total` = 지은 랭크 수 / 표의 랭크 수. rng 0 · 상태 불변 |
| **`construct(state, buildingId)`** | `→ {ok, rank}` / `{ok:false, err}` | **다음 랭크를 짓는다** [R137] — **한 칸씩 · 즉시 · 되돌림 없음**(construction_draft 원칙 5). 판정은 `constructionState` 와 같은 것 하나다 — err: `missing` · `maxRank` · `pending` · `locked` · `gold` · `materials`. 비용을 치르고(`gold` · `dust` · `stigma` 는 `resources` · 그 밖은 `materials`) 랭크 +1 · **편성을 새 상한에 맞춘다**(`deserialize` 와 같은 맞추기 — 편성 수 · 물약 칸이 더하기로 늘면 빈 자리를 곧바로 붙인다 · 2단계 · **부대 자리 `runs` 도 편성 수로** — R156) · rng 0 |
| **`hasFeature(state, id)`** | `→ bool` | **창구 — 그 기능이 열렸나** [R137] — 지어진 랭크의 여는 것(`construction.opened`)에 그 켜기가 있나(`openAll` 이 켜져 있으면 모든 건물의 최대 랭크까지). **켜기 대상이 아닌 이름은 throw**(오타를 닫힌 기능으로 두지 않는다). **기능 자리마다 한 줄씩 묻는다** [2단계 2026-09-22] — 막히면 그 자리의 거절 코드는 **`unbuilt`**(§3) · 상태 함수는 `open` 칸을 낸다. 자리: ~~`canDepart`(expedition)~~(2026-09-25 · R161) · ~~`nextRepeat`(repeat) · `stageLevelState` · `setStageLevel`(stage_level)~~(2026-09-24 · R152 — 처음부터 열림 · 어휘에서도 빠졌다) · `upgradeState` · `upgradeItem`(upgrade_item) · `makeLevels` · `makeState` · `makeItem`(make) · `moveToStash`(storage — 꺼내기는 안 막는다) · `tavernState` · `hire` · `tavernReroll`(hire) · `searchState` · `searchSend`(search) · `shopState`(shop · shop_special) · `commissionState` · `commissionFill` · `commissionTake`(commission_board — 수령 · 포기는 안 막는다: 받아 둔 것은 끝까지 간다) |
...
| `closeRun(state, now)` | `→ notice 또는 null` | **끊기** [개정 2026-09-14 · R89 · **다부대 2026-09-23**] — 재접속 · 멈춤(JS 가 오래 멈췄다)에서 부른다. **도는 부대를 전부 끊는다**(편성 번호 순 · 진행 중이던 라운드는 버리고 — 그 라운드의 물약도 안 줄어든다 · 리포트 `reason: 'closed'`). 반복도 전부 끈다 — **오프라인에는 원정이 돌지 않으므로**(base_expedition_design §1 · GAME_DESIGN §1-1) 부대가 몇이든 한 번에 걷힌다. 끊었거나 반복이 켜져 있던 부대가 하나라도 있으면 `notice` 를 세팅해 돌려주고(`notice.runs` = 끊은 부대마다 한 줄), 아니면 `null`. **남은 라운드를 마무리하지 않는다** — 마무리해 주면 껐다 켜기로 원정을 무한히 빨리 돌릴 수 있다(§8 항목 7) |
| `nextRepeat(state, endedAt, no)` | `→ {stageId, preset, at}` 또는 `null` | **반복 원정의 다음 출발** [신설 2026-09-22 · 부채 #57 · base_expedition_design §1-1 · **다부대 2026-09-23 — 부대마다 따로 묻는다**] — **그 부대**(`no`)의 끝난 원정(`runs[no-1].active` 가 아니다)이 **반복이 켜져 있고**(`run.repeat` · ~~반복이 열렸고 — `hasFeature('repeat')` · R137~~ → 처음부터 열림 · 2026-09-24 · R152) **이긴 런**(그 런의 리포트 `won`)이면 같은 스테이지 · 같은 편성으로 **`at = endedAt + repeat_restart_sec × 1000`** 에 나간다 · 아니면 `null`. **관전 여부와 무관하다** — 관전 결과 띠의 세기(SCREEN_DESIGN §4-2)와 앱 시계가 둘 다 이 답대로만 출발시킨다. `endedAt` = 그 런이 **실제로 끝난 순간**(ms) — 배속이 게임 시각을 미는 탓에 로직은 모르고 화면 층이 잰다. 아무것도 안 바꾼다 · rng 0 |
| `dismissNotice(state)` | | |
...
| `hire(state, index)` | `→ {ok, hero}` / `{ok:false, err}` | err: `roster` · `gold` · `missing`(빈 칸 포함). **`counters.tavern` 을 올리지 않는다** — 산 칸만 `tavern.hired` 에 남고 나머지 명단은 그대로다 (base_expedition_design §2-4: 고용이 무료 리롤 우회로가 되지 않게).<br>**시작 장비 한 벌을 입고 온다** [2026-09-25 사용자 지시 · hero_design §1] — `newGame` 과 같은 일반 무기 + 일반 갑옷(무기 → 갑옷 · 무기는 고유 스킬을 뺀 풀) · 바로 입어 가방 칸을 안 먹는다. rng = `deriveSeed(seed ^ 0x6EA2, counters.hero)`(§5-1 — 그 영웅의 번호) |
...
| **`gambleState(state, step = 1)`** | `→ {open, chapter, mats: {ore, timber, dust}, stakes: [{step, mult, gold, open}], step, stake, batch, pays: {symbols: [{id, gold, mat}], coins: [{id, gold}]}, n}` | **도박장 화면 상태 한 덩어리** [신설 2026-09-24 · R149 · base_expedition_design 「도박장」] — **판정을 여기서 다 낸다**(`tavernState` 와 같은 규칙) · rng 0 · 상태 불변 · **시계를 안 본다**(횟수 제한이 없다 — 같은 날 충전식에서 바뀜). `open` = 도박장이 열렸나(선술집 랭크 — `hasFeature('gamble')`). `batch` = 여러 판 돌리기의 판 수(`gamble_batch_spins`). `chapter` = 진행 중인 챕터(`shopState` 와 같은 값) · `mats` = 그 챕터 단계의 **재료 id**(광석 = `mine_node.csv` · 목재 = `log_node.csv` 의 그 tier 산출물 · 가루 = `'dust'`) — 그 tier 가 없으면 그보다 낮은 가장 높은 tier. `stakes` = 판돈 단계마다 `gold`(= `round(gamble_stake_gold × gamble_stake_chapter_mult^(chapter−1) × mult)`) · `open`(앞에서부터 `limitsOf(state).gambleStakes` 개). `step` · `stake` = 고른 단계와 그 판돈(없는 단계면 1 단계). `pays` = **그 판돈에서의 배당표** — 골드 심볼 · 코인은 `floor(값 × stake)` 골드 · 재료 심볼은 `pay × mult` 개. `n` = 지금까지 돌린 판 수(`counters.gamble`) |
| **`gambleSpin(state, step)`** | `→ {ok, spin}` / `{ok:false, err}` | **한 판** [신설 2026-09-24 · R149]. **횟수 제한이 없다 — 판돈만 있으면 돈다**(같은 날 사용자 「횟수 무제한 · 돈만 있으면」 — 충전식에서 바뀜). err: `unbuilt`(도박장이 안 열렸다) → `missing`(표에 없는 단계) → `locked`(그 단계가 안 열렸다) → `gold`(판돈이 모자란다) — **이 순서로 본다** · 거절이면 아무것도 안 바뀐다(스트림도 안 연다). 통과하면: 판돈을 빼고 · `counters.gamble` 선증가 → rng = `deriveSeed(seed ^ 0x6A3B, counters.gamble)`(§5-1) → `gamble.spin(rng)` → **골드 = `floor(goldMult × stake)`** 를 주고 · 재료 = `mats × 단계 배수` 를 준다(광석 · 목재는 `materials[그 챕터 단계 id]` · 가루는 `resources.dust`). `spin` = `gamble.spin` 의 결과 + `{n, step, stake, chapter, gold, mats: {id: n}, net, coinGold: {coinId: 골드}}` — `net` = `gold − stake`(**골드만** — 재료는 순손익에 안 든다) · `coinGold` = 코인마다 **이 판의 판돈에서** 받는 골드(`floor(value × stake)` — 화면이 코인 칸에 적는 값). **같은 세이브 · 같은 판 번호면 같은 결과다** — 결과를 저장하지 않고 다시 부르면 다음 판이다(새로고침으로 되굴릴 수 없다). **장비 · 낙인은 안 나온다**(base_expedition_design 「도박장」) |
| **`gambleSpinBatch(state, step)`** | `→ {ok, spins, stop, gold, stake, net, mats: {id: n}}` / `{ok:false, err}` | **여러 판 돌리기** [신설 2026-09-24 · R149 · 같은 날 「모두 돌리기」(충전을 다 쓴다)에서 바뀜] — `gambleSpin` 을 **`gamble_batch_spins` 판까지** 거듭 부르고 거절이 나면 거기서 멈춘다(판돈이 떨어지면 덜 돈다). 한 판도 못 돌았으면 그 거절 · `stop` = 멈춘 사유(다 돌았으면 `null`) · 합계(골드 · 판돈 · 순손익 · 재료)는 여기서 낸다(화면이 더하지 않는다). rng 는 판마다 `gambleSpin` 과 같다 |
| **`commissionState(state)`** | `→ {open, slots, taken, cards: [{…card, taken, have, done, canTake}]}` | **의뢰 게시판 한 벌** [신설 2026-09-24 · R153 · base_expedition_design §1-3] — **판정을 여기서 다 낸다** · rng 0 · 상태 불변. `open` = 게시판이 열렸나(`hasFeature('commission_board')` — 선술집 r2 · 2026-09-25 · R162) · `slots` = `limitsOf(state).commissionSlots` · `taken` = 받아 둔 수. `cards` = **게시판 자리 순서 그대로**(받은 카드도 제자리에 선다 — 자리 수 = `commission_board_cards`) · 카드 = 굴린 순간의 내용 `{no, tpl, kind, axis, ref, need, grade, gold}`(§2-16 `roll`) + `taken`(받아 뒀나) · `have`(센 수 — 안 받았으면 `null`) · `done`(`have ≥ need`) · `canTake`(열렸고 · 안 받았고 · 받은 수 < `slots`) |
| **`commissionFill(state)`** | `→ {ok, rolled}` / `{ok:false, err:'unbuilt'}` | **빈 자리를 굴려 채운다** [R153] — 자리 수가 `commission_board_cards` 가 될 때까지 끝에 붙인다 · 이미 차 있으면 `rolled: 0`(아무것도 안 바뀐다). **화면이 선술집을 그릴 때 부른다** — 선술집을 처음 지은 뒤 · 관리자 모드를 켠 뒤에도 게시판이 빈 채로 서지 않게. 카드마다 `counters.commission` 선증가 → rng = `deriveSeed(seed ^ 0xC0DA, counters.commission)`(§5-1) → `commission.roll(rng, ctx)`. `ctx` = **지금 들어갈 수 있는 스테이지**(`stageUnlocked`)의 챕터 · 일반몹(`battle.stagePool`) · 그 종족 + 진행 챕터(`frontierChapter`) |
| **`commissionTake(state, no)`** | `→ {ok, card}` / `{ok:false, err}` | **받는다** [R153] — err: `unbuilt`(게시판이 안 열렸다) → `missing`(그 번호의 **안 받은** 카드가 없다) → `full`(받은 수가 `slots` 에 닿았다) — 이 순서로 본다 · 거절이면 아무것도 안 바뀐다. 통과하면 그 카드에 `have = 0` 을 박는다 — **자리는 그대로**이고 새 카드를 안 굴린다(자리가 비는 것은 수령 · 포기 때다). **받은 뒤부터 센다** — 받기 전의 처치 · 드롭은 안 들어간다 |
| **`commissionClaim(state, no)`** | `→ {ok, gold, card}` / `{ok:false, err}` | **수령** [R153] — err: `missing`(그 번호의 받은 카드가 없다) → `notDone`(`have < need`). 통과하면 `resources.gold += card.gold` · 그 자리에 **새 카드를 굴려 넣는다**(게시판이 닫혀 있으면 자리만 지운다). **`unbuilt` 로 안 막는다** — 받아 둔 것은 끝까지 간다(수색과 같은 규칙) |
| **`commissionDrop(state, no)`** | `→ {ok}` / `{ok:false, err:'missing'}` | **포기** [R153] — **벌이 없다**(진행도만 버린다). 그 자리에 새 카드를 굴려 넣는다(`commissionClaim` 과 같다). ⚠ 받고 곧바로 포기하면 무료 리롤이다 — 기획 §10 「의뢰 세부」 ② |
...
| `searchTake(state, now)` | `→ {ok, hero, cost}` / `{ok:false, err}` | 수령(고용) — 밑값은 명단과 같고(`tavern_hire_cost`) **만남에서 고른 답이 거기서 깎는다**. err: `none`(나간 수색이 없다) · `notDone` · `roster` · `gold`(**깎인 값** 기준). 받으면 `search = null` 이라 칸이 비고 다시 보낼 수 있다. 온 영웅은 `hire` 와 같은 **시작 장비 한 벌**을 입는다(같은 스트림 · 2026-09-25) — 결과 스트림(`^ 0x5EA7`)을 안 밀어 `searchState.result` 가 보여 준 사람이 그대로 온다 |
...
| `masteryState(state, uid)` | `→ {points, nodes:[{id, treeKind, ownerId, tier, stat, name:{ko,en}, value, rank, maxRank, unlockLevel, unlocked, total, canLearn, gate, on}]}` / `null` | **판정을 여기서 다 낸다** — 화면은 결과만 그린다. 없는 영웅이면 `null`. `name` 은 `mastery_node.csv` 의 노드별 표시명이다. `gate` = 켜는 장비(`{slot, groups}` 또는 `null`) · `on` = 지금 낀 장비로 켜졌나(게이트 없으면 `true`) — 꺼진 칸도 `canLearn` 은 그대로다(랭크는 캐릭터에 쌓인다 · 2026-09-22 R138) |
...
**의뢰가 세는 곳** [신설 2026-09-24 · R153] — **`settleRound` 가 이긴 라운드를 정산할 때** 받아 둔 카드마다 센다(다 찬 카드는 더 안 센다 · `need` 에서 자른다). **처치** = 라운드 요약의 `killGrades`(§2-6)를 `{chapter: 그 런 스테이지의 챕터, monster, race: monster_type, grade}` 로 편 것 · **드롭** = 그 라운드에 **들어온** 아이템(가방에 든 것 + 알아서 분해로 곧바로 녹은 것 — 가방이 차서 버린 것은 빼고) — `commission.matchKill` · `matchDrop`(§2-16). **모든 부대가 같이 채운다**(의뢰에는 파티가 없다) · 진 라운드 · 끊긴 라운드의 몫은 안 센다(도감 처치 수와 같은 규칙) · rng 0
...
**표 셋** [2026-09-22 · R136] — 스킬 = 나가는 방식(`cast`) · 대상 · 쿨 · 조건 · 표시 / 하는 일 = 그 스킬이 차례로 실행하는 줄(`seq` 순) · 배율 · 타수 · 능력치 계수 / 걸린 효과 = `apply` 줄이 거는 창의 능력치 · 값 · 시간 · 원소 · 라운드 경계 규칙. **한 스킬은 줄을 여럿 가질 수 있다** [2026-09-24 · R151 · PLAN_skill_structure 2단계 — ~~1단계 제약 「정확히 1개」~~] — 줄마다 제 대상(`target` · `-` = 스킬 대상)을 갖고 전투는 줄을 `seq` 순으로 실행한다(§2-6 「여러 줄」). 여러 줄을 쓰는 스킬은 지금 결투(`kni_duel` — 지목 + 시전자 피해 감소) 하나다. ⚠ **설명창 재료(`previewOf`)와 화면은 아직 첫 줄(`effects[0]`)만 읽는다** — 줄마다 문장은 SCREEN_DESIGN §2 먼저(`/ui` · PLAN 2단계 「하지 않는 것」). 전투 쪽(오오라 · 사건 스킬)은 줄 목록을 다 돈다.
...
| `castable(def, ctx)` | `→ bool` | `ctx = {self, allies}`(allies = 생존 아군, self 포함). **차례에 고르는 나가는 방식이 아니면 거짓**(§2-11 `CASTS[cast].picked` — `aura` · `event` · 2026-09-24 R151 — ~~`on_death` 「늘 거짓 조건」 자물쇠~~) → 아래 발동 조건 |
...
| `scaleDef(def, stats, opt?)` | `→ eff \| null` | **스킬 계수 공용 계산 — 전투와 미리보기가 같은 함수를 쓴다** [2026-09-10 · R72 · skill_design §13]. 반환 = `def` 의 얕은 복사본 + **`effects` = 하는 일 줄마다 민 복사본**(**시전 단위** `x` — 줄마다 제 슬롯으로 민다 · 2026-09-22 R136). `x` = 그 줄 + 스킬의 `id` + **그 줄의 대상 `target`**(줄의 `target` · `-` 면 스킬의 것 — 2026-09-24 R151) + 실효 `hits`·`decay`·`procChance`·`procMult` + **`statMult`**(능력치 계수) + **`apply` 줄이면 거는 걸린 효과를 풀어 실효 `stat`·`value`·`dur`·`element` · `roundEnd`**(값 · 시간은 그 줄의 슬롯이 민다 — S5 · `status` = 걸린 효과 id 가 곧 창의 열쇠다 — §2-6 `buffs`). 실행 함수(§2-11 대상 표 · §2-12 `castHeal`·`castBuff`·`castSummon`·`castCall`)는 **`x` 하나만** 받는다. field 마다 `Σ = Σ stats[attr] × coef`(그 줄의 그 field 슬롯 전부):<br>· `mult_pct` → **배율에 안 더한다** — 그 슬롯 능력치마다 `formula.statCoef(stats[attr])` 를 곱해 `statMult` 로 낸다(**`coef` 칸은 안 읽는다** · 공격 `atk × 배율 × statMult` · 회복 `matk × mult × statMult` · 소환 `hpMax × mult × statMult` — battle_design §9-2 · 2026-09-18 — ~~`flat = Σ` 덧셈~~ 폐기) · 몬스터도 같다(2026-09-22 — 보류 해제)<br>· `hits` → `floor(raw + Σ)`, `hit` 줄은 1 이상<br>· `effect_value`·`duration_sec`·`proc_chance_pct`·`proc_mult_pct` → **크기에 더하고 부호 유지** `sign(raw) × (\|raw\| + Σ)`(raw 0 은 +) — 음수 디버프(페니턴스·바인드)가 약해지지 않는다<br>· `decay_pct` → `Σ > 0` 일 때만 `max(raw, min(raw + Σ, [balance.csv:skill_decay_cap_pct]))` — **슬롯이 민 몫만** 상한에 걸린다(CSV 원값의 0~100 미만 검증은 그대로)<br>· `stats` 가 `null`(소환·모름)이거나 그 능력치가 없으면 0 → 원값 그대로 · `statMult` 1. `mult`·`cool` 은 원값 그대로 · **rng 0 · 입력을 안 바꾼다** |
| `previewOf(def, ctx)` | `→ {baseSec, everySec, lossPct, amount, parts} \| null` | 설명창 재료 — 아래 「미리보기」 표 [2026-09-08 · **`parts` 신설 2026-09-10 R72**]. `def` 가 없으면 `null`. **첫 줄(`effects[0]`)에서 낸다** — 모양은 표 셋 분리 전과 같다(줄마다 `rows` 로 넓히는 것은 SCREEN_DESIGN §2 먼저 · `/ui` · 2026-09-24 R151 — 여러 줄 스킬(결투)도 첫 줄로 문장을 만든다) |
| `statuses` | `{statusId: status}` | **걸린 효과 정의** [신설 2026-09-22 · R136] — `status = {id, stat, value, dur, element, roundEnd, note}`(`skill_status.csv` 한 행 · `roundEnd` 2026-09-24 R151). 설명창이 `apply` 줄이 거는 효과의 능력치 · 값 · 시간 · 원소를 여기서 읽는다 |
...
**`line`**(하는 일 한 줄 · `skill_effect.csv`) — `{seq, effect, target, hits, mult, decay, procChance, procMult, element, status, scales:[{field, attr, coef}]}` · `effects` 는 `seq` 순 · **`target` = 그 줄의 대상**(`-` 는 `null` = 스킬 대상 · 2026-09-24 R151) · `status` = `apply` 줄이 거는 걸린 효과 id(그 밖은 `null`) · `element` = `hit` 줄의 원소(`apply` 줄의 원소는 걸린 효과가 든다).
...
| `cast`(skill.csv) | **나가는 방식** — §2-11 `CASTS` 의 키: `turn`(차례에 쓴다) · `aura`(쿨 없이 상시 · 행동을 안 먹는다 — 전투 시작에 `until: Infinity` 창으로 걸린다) · `event`(사건이 부른다 — `cast_condition` 이 그 사건(§2-11 `EVENT_TRIGGERS`)이고 차례에는 안 고른다 · 실제 발동은 그 사건이 나는 자리 — `on_death` = `battle.js` `downed` 끝의 `rt.fire`) [2026-09-22 · 옛 `kind` 의 「어떻게 나가나」 절반 · 표가 규칙을 든다 2026-09-24 R151] |
...
| `target`(skill.csv · skill_effect.csv) | `enemy_single` · `enemy_all` · `enemy_rotate` · `enemy_chain` · `enemy_highest_def` · `self` · `party` · `ally_single` · `party_adjacent` · **`enemy_hp_max`** — §2-11 `TARGETS`(= 공격 대상 ∪ 고르는 대상). **이름 하나에 뜻 하나** [2026-09-24 · R151] — `enemy_single` 은 **공격**(전열 우선 무작위 · rng 1)뿐이고, 적에게 거는 창의 「생존 적 중 HP 최대」(rng 0)는 `enemy_hp_max` 다(결투). `skill_effect.csv:target` 은 **그 줄의 대상**이고 `-` 면 스킬의 `target` 이다(결투 둘째 줄 = `self`) |
...
| `status`(skill_effect.csv) | `apply` 줄 = 거는 걸린 효과의 `status_id`(없으면 throw) · 그 밖의 줄 = `-`. **대부분 `status_id` = 스킬 id 다**(`mag_focus` — 한 스킬이 효과 하나를 건다) · 한 스킬이 효과를 둘 이상 걸면 둘째부터 제 이름을 갖는다(`kni_duel_guard` · 2026-09-24 R151) · 여러 스킬이 같이 거는 효과(화상 등)도 제 이름을 갖는다. **이 id 가 창의 열쇠다**(§2-6 `buffs`) |
| `cast_condition` | `turn` · `aura` 스킬 = `-` · `buff_absent` · `ally_hp_below` · `band_missing`(§2-11 `CONDITIONS`) / **`event` 스킬 = 사건 이름** `on_death`(§2-11 `EVENT_TRIGGERS` · 비면 throw) [2026-09-24 · R151 — 두 어휘가 갈렸다. 차례 스킬에 사건 이름 · 사건 스킬에 조건을 적으면 throw] |
| `round_end`(skill_status.csv) | **`keep`** · **`close`** [신설 2026-09-24 · R151] — `close` 창은 라운드가 바뀌면 닫힌다(§2-6 「라운드 경계」 — 파티만 · `buffEnd`). 지금 `close` 는 결투의 시전자 창(`kni_duel_guard`) 하나다. **오오라(`cast=aura`)가 거는 효과는 `keep` 이어야 한다**(상시 창이 라운드마다 꺼지면 다시 켜는 자리가 없다 · throw) |
...
**태그는 14종이고 컬럼에 적는 것은 11종뿐이다** (skill_design §11 확정 2026-08-28 · `aura` 2026-09-08). 나머지 셋은 **`hit` 줄 + 그 줄의 대상**(줄 `target` · `-` 면 스킬 `target`)에서 **파생**한다 — `aoe`(`enemy_all`·`enemy_chain`) · `single`(`enemy_single`·`enemy_highest_def`) · `multihit`(그 줄의 `hits > 1`) · `hit` 줄이 없으면 파생 태그도 없다. `enemy_rotate`(순환)는 타수만큼만 닿으므로 **광역도 단일도 아니다**(§11-2 규칙 3). 파생 가능한 것을 컬럼에 또 적으면 두 곳 관리가 되어 반드시 어긋나므로, `tags` 에 파생 태그를 적으면 **로드가 실패한다**.

로드 시 그 밖에 던지는 것 — `hit` 인데 `hits < 1` · `turn` 스킬의 `apply` 줄이 거는 걸린 효과의 `duration_sec ≤ 0` · `aura` 스킬의 걸린 효과의 `duration_sec ≠ 0`(오오라는 창이 아니다) · `apply` 인데 걸린 효과가 없음 · `heal` 인데 `mult_pct ≤ 0` · `turn` 인데 `cool_sec ≤ 0` · `aura`·`event` 인데 `cool_sec ≠ 0`(규칙은 §2-11 `CASTS[cast].cooled`) · `event` 인데 `cast_condition` 없음 · 사건 어휘 밖 · `turn`·`aura` 인데 조건 어휘 밖 · 걸린 효과의 `round_end` 어휘 밖 · 오오라가 거는 효과가 `close` · `skill_id` 중복 · `owner_kind` 어휘 밖 · `owner_id` 빈 값 · **같은 출처(`owner_kind#owner_id`) 안 `priority` 중복** · `tags` 가 3개 이상 · 어휘 밖 태그 · 파생 태그를 적음 · 태그 중복.
**[강화 2026-09-01 · 표 2026-09-24 R151] 하는 일↔대상 정합**(대상은 **그 줄의 대상** — 줄 `target` · `-` 면 스킬 `target`) — `hit` 은 **공격 대상**(§2-11 `ATTACK_TARGETS` 의 키)만 · `fixed` 는 **적 쪽 고르는 대상**(`PICK_TARGETS` · `side = enemy`)만 · `heal` 은 **아군 쪽 고르는 대상**(`side = ally`)만 · `aura` 스킬의 `apply` 는 **`self` · `party` 만**(오오라는 고르는 대상 표를 안 거친다 — `battle.js:applyAuras` 가 자기 또는 **편 배열 전체**에 건다 · 2026-09-24) · `turn` 스킬의 `apply` 는 고르는 대상 전부(아군 + 적에게 거는 창 — 음수 값 · 2026-09-09) · `summon`·`call` 은 `self` 만 · 줄 `target` 이 어휘 밖이면 throw · `hit` 의 `enemy_all`/`enemy_chain` 은 `hits = 1`(타수는 대상 수가 정한다) · `heal`/`apply`/`summon`/`call` 은 `hits = 0` · `apply`/`call` 은 `mult = 0` · `hit`·`heal`·`summon` 은 `mult > 0` · `apply` 가 아닌 줄의 `status` 는 `-` · `ally_hp_below` 는 `0 < cond_value ≤ 1`, 그 외 조건은 `cond_value = 0`. 컬럼 하나가 대상마다 뜻이 달라 값이 **조용히 무시되던** 자리를 로드 시 막는다.
**[신설 2026-09-22 · R136] 표 사이 검사** — ① 모든 스킬에 하는 일 줄이 **1개 이상** 있다(~~1단계는 정확히 1개~~ — 2026-09-24 R151) · `seq` 는 스킬마다 1 부터 빈틈없이 ② 없는 스킬 id 를 가리키는 줄 금지 ③ 없는 걸린 효과를 가리키는 `apply` 줄 금지 ④ **아무도 안 거는 걸린 효과 금지**(읽히지 않는 행) · 걸린 효과 쪽 — `status_id` 유일 · `stat` 이 `EFFECTS` 의 키 · `value`·`duration_sec` 숫자(`duration_sec ≥ 0`) · `element` 어휘. 전부 어긋나면 throw.
**[강화 2026-09-10 · R72] 슬롯 · 추가 피해 · 감쇠 · 결투** — ① 슬롯(하는 일 줄마다): `field` 어휘 밖 · `attr` 이 `hero_attribute` id 밖 · `coef` 가 숫자가 아니거나 음수 · `field='-'` 와 `attr='-'` 가 짝이 안 맞음 · 빈 슬롯인데 `coef ≠ 0` · **한 줄에 같은 field 두 번** · 하는 일 부적합(`hits`=hit · `mult_pct`=hit/heal/summon · `effect_value`=apply · `duration_sec`=`turn` 스킬의 apply · `decay_pct`=감쇠를 쓰는 줄 · `proc_*`=확률 > 0 인 hit). **채운 슬롯의 `coef = 0` 은 합법**이다(축만 정하고 크기는 밸런스 몫 — skill_design §13-3) ② 추가 피해: `proc_chance_pct` 는 0~1 · 0 보다 크면 `effect=hit` 이고 `proc_mult_pct ≥ 1` · 0 이면 `proc_mult_pct = 0`(두 곳 관리 금지) ③ 감쇠: `decay_pct` 를 쓰는 줄은 **`hit` 줄이고 그 대상이 §2-11 `HIT_DECAY` 에 있다**(`enemy_chain` · `enemy_highest_def` · `enemy_all` — 광역 약화 2026-09-10 · 표 2026-09-24)이고 그 밖은 0 ④ 걸린 효과 `stat = duel` 의 `value ≥ 0`(시전자가 받는 피해 감소 % — 실제 감소는 결투 둘째 줄 `kni_duel_guard` 가 걸고, 지목 행의 값은 `buff` 이벤트 `v` · 설명창 문장이 읽는다 · ⚠ 두 행에 같은 값 — 줄마다 문장이 오면 둘째 줄이 제 값을 말한다 · 2026-09-24 R151).
...
|---|---|
| `-` | 항상 |
| `buff_absent` | 시전자에게 **이 스킬이 건 창이 없다**(창의 `s` 로 본다 — 창 열쇠가 걸린 효과 id 라서 · 2026-09-24 R151) |
...
**사건** — `event` 스킬의 `cast_condition` 은 조건이 아니라 **사건 이름**이다(§2-11 `EVENT_TRIGGERS`). 차례에는 안 고르고(`castable` 이 거짓) 그 사건이 나는 자리에서 `rt.fire` 가 쏜다 [2026-09-24 · R151 — ~~`on_death` 는 「늘 거짓」 조건 자물쇠 · 발동은 `battle.js:blast`~~].

| 사건 | 나는 자리 | 규칙 |
|---|---|---|
| `on_death` | `battle.js` `downed` 끝(처치 정산 · `down` 훅 뒤) | `once` — 한 유닛 한 번(`fired` · 되살아나도 안 풀린다 · §2-6 자폭 행) [신설 2026-09-21] |
...
**표준 모양** [2026-09-24 · R151 · PLAN_skill_structure 2단계] — 스킬(`skill.csv` · WoW Spell) → 하는 일 줄(`skill_effect.csv` · SpellEffect — 줄마다 `EFFECT_TYPES` 하나 + 대상 하나) → 걸린 효과(`skill_status.csv` · Aura / Dota modifier — `EFFECTS` 하나). 대상은 **공격 대상**(누구를 · 몇 번 · 어떤 배율 — 굴림이 든다)과 **고르는 대상**(고르기만 — 결정론) 두 표이고, 능력치 창은 **능력치마다 한 곳에서 모아** 파생한다(GAS attribute aggregator). 스킬 id 전용 실행 코드는 **없다**(사용자 09-24 — 전용 스크립트 탈출구를 두지 않는다).

| export | 모양 | 계약 |
|---|---|---|
| `CASTS` | `{ turn, aura, event }` — 각 `{picked, cooled}` | `skill.csv:cast` 허용값 = 키(나가는 방식 · §2-8) [표 2026-09-24 R151 — ~~`['turn','aura','event']` 배열~~]. **`picked`** = 차례에 고르나(`skill.castable` 이 먼저 본다 — 거짓이면 조건과 무관하게 준비가 아니다) · **`cooled`** = `cool_sec > 0` 이어야 하나(아니면 0 이어야 한다 · 로드 검사). `turn` = 고른다 · 쿨 양수 / **`aura`** = 쿨 없이 상시(§2-6 — 액티브 칸에서 빠져 전투 시작에 `until: Infinity` 창으로 걸린다 · 2026-09-09) / **`event`** = 사건이 부른다(`EVENT_TRIGGERS` · 2026-09-21) |
| `CAST_IDS` | `Object.keys(CASTS)` | 나가는 방식 어휘 |
| `EVENT_TRIGGERS` | `{ on_death }` — 각 `{once}` | **사건** [신설 2026-09-24 · R151] — `event` 스킬의 `cast_condition` 허용값 = 키. **`once`** = 한 유닛 한 번(`u.fired` · 되살아나도 안 풀린다). 쏘는 자리는 사건을 아는 쪽이다 — `on_death` = `battle.js` `downed` 끝의 `rt.fire(u, 'on_death', t)`(§2-6 자폭 행) |
| `EVENT_IDS` | `Object.keys(EVENT_TRIGGERS)` | 사건 어휘 |
| `EFFECT_TYPES` | `{ hit, heal, apply, summon, call, fixed }` — 각 `{casts, foes?, run}` | `skill_effect.csv:effect` 허용값 = 키(하는 일 · §2-8). `casts` = 그 일을 싣는 나가는 방식(`skill.js` 가 검증한다) · **`foes: true`** = 살아 있는 적이 없으면 그 줄을 건너뛴다(`hit` · `fixed` — §2-6 「여러 줄」) · `run(rt, u, x, t, foes)` = 한 줄의 실행 — `hit` → `ATTACK_TARGETS[x.target]` · `heal` → `rt.castHeal` · `apply` → `rt.castBuff` · **`summon`** → `rt.castSummon`(HP 를 가진 유닛을 세운다 · 2026-09-09) · **`call`** → `rt.castCall`(시전자의 무리(§2-13 `band`) 중 서 있지 않은 것을 **한 번에 전부** 세운다 · 몬스터 전용 · 2026-09-18) · **`fixed`** → 비직격 고정 피해 — 세기 `F.indirect((atkMin + atkMax) / 2 × x.mult × x.statMult)`(0 이하면 아무 일도 없다) · 대상 = `PICK_TARGETS[x.target]` · 맞히는 일은 `rt.dealIndirect`(battle.js 이음매) [`run` 신설 2026-09-24 R151 — ~~`battle.js:blast` 가 실행~~] |
| `EFFECT_IDS` | `Object.keys(EFFECT_TYPES)` | 하는 일 어휘 |
| `ATTACK_TARGETS` | `{ enemy_single, enemy_all, enemy_rotate, enemy_chain, enemy_highest_def }` — 각 `(rt, u, x, foes) → void`(`x` = `hit` 줄의 시전 단위) | **공격 대상** 5종의 **실행 자체**(§2-6 타겟팅 행 그대로 — rng 순서·hp 가드 불변) — 누구를 · 몇 번 · 어떤 배율로. `rt` = §2-12 런타임. **`enemy_highest_def`** [2026-09-09] = 방어값 최대 대상(동률이면 배열 순 앞 — **rng 0회**)을 `hits` 회 때리고 타격마다 그 대상의 `def`·`defBase` 를 `decay` 만큼 곱으로 깎는다. **`enemy_all`** 은 `decay > 0` 이면 주 대상(전열 생존자 중 배열 첫 번째 · 전열이 비면 생존자 첫 번째 · **rng 0회**)만 온전하고 나머지 배율이 `decay` 만큼 준다 — `decay = 0` 이면 종전과 같다 [2026-09-10 · R72]. 핸들러는 전부 스킬 타격에 `{statMult, procChance, procMult}` 를 `rt.strikeOnce` 의 6번째 인자로 넘긴다 |
| `HIT_DECAY` | `{ enemy_all: 'secondary', enemy_chain: 'by_order', enemy_highest_def: 'def_shred' }` | **공격 대상이 `decay` 를 무슨 뜻으로 쓰나** [신설 2026-09-24 · R151 — ~~`skill.js:usesDecayOf` 하드코딩~~] — 주 대상 밖 약화 · 순서마다 곱으로 감소 · 타격마다 대상 방어 감소. 여기 없는 대상의 `hit` 줄은 `decay_pct = 0` 이어야 한다(로드 검사 · 슬롯 `decay_pct` 도 여기 있는 줄만) |
| `PICK_TARGETS` | `{ self, party, ally_single, party_adjacent, enemy_all, enemy_hp_max }` — 각 `{side, pick(rt, u) → 유닛[]}` | **고르는 대상** [신설 2026-09-24 · R151 — ~~`SUPPORT_TARGETS` · `DEBUFF_TARGETS` 배열 + `skill_runtime.targetsOf` 의 switch~~] — `heal` · `apply` · `fixed` 가 쓴다(오오라의 `apply` 는 아니다 — §2-8 「하는 일↔대상 정합」). **전부 결정론(rng 0회)**. `side = ally`: `self` · `party`(생존 아군) · `ally_single`(HP **비율** 최저 — 사용자 확정 2026-09-09) · `party_adjacent`(`party` 배열의 양 옆 · 자기 제외 — 위치는 임시 규칙). `side = enemy`(**적에게 거는 창** — 같은 창을 **음수 `effect_value`** 로 쓴다 · 사용자 확정 2026-09-09): `enemy_all`(생존 적 전원 · 배열 순) · `enemy_hp_max`(생존 적 중 **HP 최대** — 결투의 지목 · 동률이면 배열 순 앞). 어느 하는 일이 어느 쪽을 쓰나는 §2-8 「하는 일↔대상 정합」 |
| `TARGETS` | 공격 대상 ∪ 고르는 대상 | `skill.csv:target` · `skill_effect.csv:target` 허용값 (`enemy_all` 은 두 표에 다 있다 — 공격이면 광역 타격, 창이면 적 전원) |
| `EFFECTS` | `{ atk_pct, period_pct, barrier_pct:{apply}, guard_pct, def_pct, res_elem, hp_max_pct:{apply}, regen_pct, dr_pct, onhit_element:{state}, attack_splash:{state}, duel:{state}, taunt:{state} }` | **걸린 효과의 능력치 어휘**(`skill_status.csv:stat` · 2026-09-22). 셋 중 하나다 [2026-09-24 · R151 — ~~각 stat 이 제 `derive(u, sum)` 를 들었다~~]: ① **창 합이 능력치를 민다** — 어느 능력치를 어떻게 미나는 아래 `refreshDerived` 의 능력치 표가 든다(한 능력치를 여러 효과가 밀면 **그 능력치 한 곳에서** 모은다) ② **`state: true` = 표식** — 파생값이 없고 읽는 쪽이 `stateOf` · `sumOf` 로 본다: `taunt` · `duel` 의 소비자는 `battle.pickTarget` · `onhit_element` · `attack_splash` 의 소비자는 `skill_runtime.basicAttack` ③ **`apply(rt, tgt, x, until, ev)`** = 시전 순간 1회 — `barrier_pct`(HP 밖 흡수 풀 · 창이 끝나면 사라진다) · `hp_max_pct`(열 때 늘어난 만큼 현재 HP 도 올린다 · 닫을 때 넘친 HP 는 파생이 깎는다). **`def_pct` · `res_elem` 은 무기 옵션 창 전용**(스킬 행이 안 쓴다) · `res_elem` 은 창의 `element` 칸만 민다 [2026-09-11 · R78] |
| `EFFECT_STATS` | `Object.keys(EFFECTS)` | `skill_status.csv:stat` 허용값 |
| `CONDITIONS` | `{ buff_absent, ally_hp_below, band_missing }` — 각 `(def, ctx) → bool` | 발동 조건(§2-8 표) — `turn` · `aura` 스킬의 `cast_condition` 허용값. `ctx = {self, allies}` · `buff_absent` 는 창의 `s`(건 스킬)로 본다 [2026-09-24 · R151 — ~~`on_death` 상수 거짓~~ 은 `EVENT_TRIGGERS` 로 갔다] |
| `CONDITION_IDS` | `Object.keys(CONDITIONS)` | 조건 어휘 |
| `refreshDerived(u)` | `→ void` | **능력치 표를 순서대로** 다시 쓴다 [개정 2026-09-24 · R151 — ~~`EFFECTS` 의 키 순서대로 stat 마다 `derive` · 순서가 계약~~(`def_pct` 가 `guard_pct` 뒤라야 둘을 다 들었다)] — 데미지 괄호(`atk_pct` → `dmgPct` · 공격 · 회복 밑수 양끝) · 주기(`period_pct`) · **방어**(`defScaled(defBase, Σdef_pct + Σguard_pct)`) · **저항**(`resBase + Σguard_pct` 에 `res_elem` 창을 창 순서대로 더한다) · 최대 HP(`hp_max_pct` · 넘친 HP 를 자른다) · 재생(`regen_pct`) · 받는 피해 감소(`dr_pct` — **창 합을 안 쓴다 · 창 하나가 원천 하나**: `dr = 1 − (1 − drBase) × reductionMult(창 값들)` · §2-3 · battle_design §9-3 · 창이 없으면 `drBase` 그대로 · 음수 창은 `1 − v > 1` 로 곱해진다 · 2026-09-22). 능력치끼리 서로 안 읽으므로 **표 순서는 결과를 안 바꾼다** · 식은 옛 판과 부동소수 연산 순서까지 같다. 창이 없으면 합 0 이라 밑수로 돌아온다 |
| `sumOf(u, stat)` | `→ number` | 한 유닛의 그 능력치 **창 합**(창 순서대로 덧셈) [신설 2026-09-24 · R151] — 평타 광역(`attack_splash`)의 배율 · 능력치 표가 쓴다 |
| `stateOf(u, stat)` | `→ [열쇠, 창] \| null` | 그 능력치의 **첫 창**(삽입 순서 — §8 항목 15) [신설 2026-09-24 · R151] — 지목(`duel` · 건 자 `by`) · 도발(`taunt`) · 원소 추가타(`onhit_element` · 원소 · 값 · 건 스킬 `s`) |
...
새 종류를 추가하는 절차 — 이 표에 항목 하나(+ 필요하면 §2-6 실행 규칙 행) → `skill.csv` · `skill_effect.csv` · `skill_status.csv` 에서 그 값을 쓴다. `skill.js` · `battle.js` 는 건드리지 않는다. 예외 하나 — **새 능력치**(지금 창이 안 미는 축 · 예: 치명 확률)를 미는 효과는 능력치 표에 한 줄 + 유닛에 그 **밑수 필드**(`battle.js:makeUnit` · 갈아입기 `REFIT_FIELDS`)가 필요하다 — 창이 없을 때 돌아갈 자리다(§2-6 전투 유닛 「밑수」 행).

### 2-12. `skill_runtime.js` — 시전 · 창 · 배리어 · 사건 훅 (2026-09-01 신설)

`battle.createRun` 이 **런마다** 만든다 — 상태(`t` · 유닛)는 전부 인자·ctx 로 받고 모듈 전역은 없다.

`createSkillRuntime(ctx)` — `ctx = { SK, B, rng, timeline, out, units:{party, enemies}, strikeOnce, pickTarget, makeSummon, callBand, dealIndirect, r1, EPS, hooks }`. **`dealIndirect(a, d, dmg, s)`** 도 **battle.js 가 넘긴다** [신설 2026-09-24 · R151] — 비직격 고정 피해 한 대상(HP 차감 · `blast` 이벤트 · 기여 · 전투불능)은 기여표와 전투불능을 아는 쪽의 일이다(§2-6 자폭 행). 런타임은 `B` 로 제 `formula` 를 만든다(`F.indirect` — 고정 피해의 반올림 · `createFormula` 는 무상태라 인스턴스를 공유하지 않아도 같다). `callBand(caster, t) → {units: [표시값], then: [t 없는 이벤트]}` 도 **battle.js 가 넘긴다** [2026-09-18] — 무리를 세우는 것은 적 배열 · 오오라 · 보상 표식을 아는 쪽의 일이다. `makeSummon(caster, def) → 유닛` 은 **battle.js 가 넘긴다** [2026-09-09] — 유닛 생성자와 키 발급(`s0`…)은 그쪽 어휘라 런타임이 만들지 않는다. `units.enemies` 는 라운드마다 simulate 가 갈아 끼우는 **속성**이라 런타임은 항상 `ctx.units.enemies` 를 읽는다. `strikeOnce(u, target, mult, element, s, sk?)`·`pickTarget` 은 battle.js 의 것을 그대로 받는다(`sk = {flat, procChance, procMult}` 는 **스킬 타격만** 넘긴다 · 2026-09-10 R72) — 직격·도발·전투불능 판정은 전투 진행의 몫이고 런타임은 **무엇을 시전하나**만 안다.

| export | 계약 |
|---|---|
| `act(u, t)` | §2-6 「발동」·「쿨」 행 — 살아 있는 적이 없으면 아무것도 안 한다 · 준비된 것 없으면 `basicAttack`. 시전이면 `readyAt` · `out.casts` · `skill` 이벤트 · `hooks.emit('cast', …)` → **`cast(u, def, t)`** |
| `cast(u, def, t)` | **시전 한 번** [신설 2026-09-24 · R151 — 차례(`act`)와 사건(`fire`)이 같이 쓴다] — **`eff = SK.scaleDef(def, u.stats)`**(시전 순간 한 번 · 2026-09-10 R72) → **하는 일 줄을 `seq` 순으로 실행** — 줄마다 살아 있는 적을 다시 보고(`EFFECT_TYPES[x.effect].foes` 면 적이 없을 때 건너뛴다) `EFFECT_TYPES[x.effect].run`(§2-11 — `ATTACK_TARGETS[x.target]` / `castHeal` / `castBuff` / `castSummon` / `castCall` / 고정 피해)이 **그 줄의 시전 단위 `x` 를 받는다**(아군 창도 적에게 거는 창도 여기 · 2026-09-22 R136 — ~~`kind` 로 가르던 if 사슬~~). `skill` 이벤트 · 시전 수 · 쿨은 **안 건드린다**(부르는 쪽 몫) |
| `fire(u, name, t)` | **사건** [신설 2026-09-24 · R151 — ~~`battle.js:blast`~~] — 그 유닛의 칸에서 `cast = event` · `cast_condition = name` 인 스킬을 **칸 순서대로** `cast` 한다. `EVENT_TRIGGERS[name].once` 면 `u.fired[스킬 id]` 가 선 스킬은 건너뛰고 쏘기 **전에** 세운다(한 유닛 한 번 · 되살아나도 안 풀린다). 부르는 자리는 사건을 아는 쪽 — `on_death` = `battle.js` `downed` 끝 · rng 는 그 스킬의 줄이 쓰는 것뿐(자폭 0) |
...
| `basicAttack(u, t, foes)` | 기본 공격 [신설 2026-09-09]. **평타 부여 창을 여기서 읽는다**(§2-11 `sumOf` · `stateOf` — 2026-09-24) — `attack_splash` 가 켜져 있으면 단일 → **적 전원**이고 배율이 창 합(없으면 1배 단일) · `onhit_element` 가 켜져 있으면 때린 대상마다 **원소 추가타 1회**(첫 창의 원소 · 값 · `s` = 그 창을 건 스킬). ⚠ 창이 하나도 없으면 **종전과 완전히 같다**(`strikeOnce(u, pickTarget(u, foes), 1, null)` 1회) |
| `targetsOf(u, x)` | heal·apply·고정 피해가 쓰는 대상 = `PICK_TARGETS[x.target].pick(rt, u)`(§2-11 — 2026-09-24 R151 · ~~런타임 안 switch~~). **rng 0회** |
...
| `expire(u, t)` | 창·배리어 만료(`until ≤ t + EPS`) → `buffEnd`(`s` = 창의 `s` — 건 스킬 id · **`quiet` 창은 안 낸다** — 무기 옵션 창 · R78) → 바뀌었으면 `refreshDerived` |
| `castHeal(u, x, t)` · `castBuff(u, x, t)` | §2-6 회복·버프 창 행. **`castHeal` 은 rng 1회**(회복량 굴림 · 대상 선택 앞 · R90). 배리어는 `EFFECTS.barrier_pct.apply`. `x` 는 **`scaleDef` 가 낸 시전 단위**다(`statMult`·실효 `value`·`dur`) — `statMult` 가 없으면 1 로 읽는다. **창을 거는 함수는 걸린 효과 행 + 거는 스킬 id 를 받는다** — **창의 열쇠 = `x.status`**(걸린 효과 id · 2026-09-24 R151 — ~~`x.id` 거는 스킬~~ · 손으로 만든 시전 단위처럼 `status` 가 없으면 `x.id`) · 창의 `stat`·`v`·`until`·`element`·`roundEnd` = 걸린 효과를 푼 `x.stat`·`x.value`·`x.dur`·`x.element`·`x.roundEnd` · 창의 `s` = `x.id`(건 스킬 — `buff` 이벤트의 `s`). 같은 효과 재시전 = 갱신 · 다른 효과의 같은 능력치 = 덧셈 — S2-a. **결투 분기는 없다** — 시전자의 피해 감소는 결투의 둘째 줄이 건다(§2-6 `duel` 행 · 2026-09-24). `until` 은 시전자의 **버프 지속시간**(`u.buffDur`)을 곱한다(§2-6 버프 창 행 · 2026-09-21) |
...
| (런타임 객체 `rt`) | `act`·`cast`·`fire`·`expire`·`castHeal`·`castBuff`·`castSummon`·`castCall`·`basicAttack`·`targetsOf`·`alive`·`alliesOf`·`foesOf` 에 더해 **`rng`·`F`·`strikeOnce`·`pickTarget`·`dealIndirect`** 도 같은 객체에 싣는다 — 등록표(§2-11)의 핸들러가 `rt.*` 만 보고 battle.js 를 import 하지 않게 하는 이음매 (~~`buffSum`~~ 은 2026-09-24 §2-11 `sumOf` 로) |
...
| export | 내용 |
|---|---|
| `TARGETS` | 여는 것의 「무엇」 `{id: {kind: 'unlock' \| 'add', live, seed}}`. `live: false` = **준비 중**(그 기능이 아직 없다 — 화면은 「준비 중」). **새 기능을 건물 뒤로 넣을 때만 한 줄 는다** — 그 기능 자리에는 `state.hasFeature` 한 줄. 더하기 대상 중 **`state.limitsOf` 의 키와 같은 이름**(`bag` · `stash` · `roster` · `presets` · `potionSlots` · `upgrade` · `searchSlots` · `shopPerSlot` · `shopWeapon`)은 그 상한에 **기본값 위로 더해진다**(2단계 2026-09-22 · `state.limitsOf`) — **`tavernCandidates` 는 어휘에 없다**(명단은 고정 · 표에 적으면 로드가 멈춘다 · 2026-09-24 사용자 지시) · **`gambleStakes`**(도박장 판돈 단계 · 2026-09-24 · R149) · **`chapters`**(들어갈 수 있는 장 · 원정 랭크 · 기본값 0 · 2026-09-24 · R152)도 같은 이름에 붙는다 · ~~`repeat` · `stage_level`~~ 은 어휘에서 빠졌다(처음부터 열림 · R152 — 표에 적으면 로드가 멈춘다) — 단계 셋 `make_level` · `potion_tier` · `tactic_slots` 는 `makeLevels` · `potionTier` · `tacticSlots` 에 붙는다(기본값 0). **첫 단계는 켜기가 심는다**(`seed` · 2026-09-24 사용자 지시 「제련소 1 이 열릴 때 기본적인 제작과 강화는 열리는 걸로」) — `make_level` 은 `seed: 'make'` · `potion_tier` 는 `seed: 'potion'`(켜기 **`potion` = 물약 제작** — 제련소 r2 · 2026-09-24 사용자 지시 「랭크 2 를 물약 제작, 제작 레벨 +1 로」)이라 그 켜기가 처음 나오는 랭크에서 **1 씩 함께 선다**(제작 Lv1 · 물약 1단계 — 표에 +1 줄을 따로 적지 않는다 · 표의 더하기는 그 위로 쌓인다 · 물약은 단계로만 잠기므로 `potion` 에 `hasFeature` 자리는 없다). `seed` 는 켜기 대상이어야 한다(아니면 생성 때 throw). 연구의 상한은 `research:<연구 id>` 로 더한다(표에 연구가 있을 때만 유효) |
...
| `opened(ranks)` | `{features: [id], adds: {target: n}}` — 지어진 랭크까지의 여는 것. **준비 중 대상도 모은다**(그 기능이 생기면 곧바로 먹게) · **켜기가 심는 첫 단계**(`TARGETS.seed`)도 `adds` 에 1 씩 든다 — 그 켜기가 처음 나올 때 한 번 |
...
| `reach(target, n = 1)` | 그 대상이 **n 에 닿는 랭크** `{id, name, rank}` · 못 닿으면 `null` [2단계 2026-09-22] — 켜기 = 그 줄이 처음 나오는 랭크(n 은 안 본다) · 더하기 = 값을 쌓아 n 이상이 되는 랭크(기본값은 안 센다 — 부르는 쪽이 뺀다 · **켜기가 심는 첫 단계는 센다** — `reach('make_level', 1)` = 제련소 r1). 여러 건물이 한 대상을 더하면 **표의 건물 순서 · 랭크 순서**로 쌓는다. `state.needOf` 가 부른다 |
...
### 2-15. `gamble.js` — 도박장 슬롯 (2026-09-24 신설 · R149)

순수 모듈 · 상태 없음 · **세이브를 모른다** — 한 판의 결과를 굴려 **데이터로** 낸다. 판돈 차감 · 지급은 `state.js` 가 한다(`gambleSpin` · §2-7). 기획은 [base_expedition_design.md](../game_design/base_expedition_design.md) 「도박장」.

`createGamble({symbols, coins, lines, stakes, balance})` — 표 넷(`slot_symbol.csv` · `slot_coin.csv` · `slot_line.csv` · `slot_stake.csv` 의 파싱 행)과 balance(`gamble_reels` · `gamble_rows` · `gamble_hold_trigger` · `gamble_hold_respins` · `gamble_hold_coin_pct`). **로드 시 검증한다**(어긋나면 throw): 심볼 id 유일 · `kind ∈ {line, wild, coin}` · `yield ∈ {gold, ore, timber, dust, -}`(coin 만 `-`) · wild · coin 은 **하나씩** · 가중치 ≥ 0 이고 합 > 0 · 코인 `kind ∈ {gold, jackpot}` · **`on_fill = 1` 인 행이 정확히 하나이고 그 행의 가중치는 0**(판이 다 찼을 때만 나온다) · 라인의 `rows` 가 `gamble_reels` 칸이고 칸마다 `0 ~ gamble_rows−1` · 판돈 단계가 1 부터 이어진다 · 배수 > 0.

| 시스템 export | 계약 |
|---|---|
| `symbols` · `coins` · `lines` · `stakes` | 정규화한 표 — 화면이 배당표를 그릴 때 읽는다 `{id, kind, yield, pay, weight, name}` · `{id, kind, value, weight, onFill, name}` · `{id, cells: [row…]}` · `{step, mult}` |
| `reels` · `rows` | 판의 크기 — 칸 = `reels × rows`, **칸 번호 = 릴 × rows + 행**(릴 먼저) |
| `spin(rng)` | `→ {cells: [symbolId], coinAt: [coinId \| null], lines: [{line, symbol, yield, pay}], hold: null \| {start: [coinId \| null], steps: [{left, landed: [{i, coin}]}], full}, goldMult, mats: {ore, timber, dust}}` — **돈 단위를 모른다.** `goldMult` = 판돈의 몇 배를 받나(라인의 골드 `pay` + 홀드의 코인 값 + 다 찼으면 `on_fill` 행) · `mats` = 재료 심볼 라인의 `pay` 합(판돈 단계 배수는 `state` 가 곱한다). 라인 판정: 그 라인의 칸에 **코인이 하나라도 있으면 안 맞는다** · 전부 와일드면 와일드가 · 아니면 와일드 아닌 심볼이 전부 같을 때 그 심볼이 맞는다(와일드는 대신 선다) · 맞은 라인마다 따로 준다. **홀드 앤 스핀**: 첫 판의 코인이 `gamble_hold_trigger` 개 이상이면 선다 — 코인은 제자리에 붙고 남은 리스핀 = `gamble_hold_respins` · 리스핀마다 빈 칸이 칸 번호 순으로 `gamble_hold_coin_pct` 확률로 코인을 받는다 · **새 코인이 하나라도 서면 남은 리스핀이 처음 값으로 돌아간다** · 0 이 되거나 판이 다 차면 끝 · 첫 판의 코인도 값을 든다(서지 않으면 버린다). rng 순서는 §5-2 |

### 2-16. `commission.js` — 의뢰 카드 굴림 · 대조 (2026-09-24 신설 · R153)

순수 모듈 · 상태 없음 · **세이브를 모른다** — 게시판 카드 한 장을 굴려 **데이터로** 내고, 처치 · 드롭 하나가 그 카드를 채우는지 답한다. 받기 · 세기 · 수령은 `state.js` 가 한다(§2-7). 기획은 [base_expedition_design.md §1-3](../game_design/base_expedition_design.md) 「게시판 · 굴림 · 수령」.

`createCommission({templates, kinds, grades, races, monsters, balance})` — 표 넷(`commission.csv` 틀 · `commission_kind.csv` 종류 · `commission_grade.csv` 의뢰 등급 · `monster_type.csv` 종족의 파싱 행) · `monsters`(byId — 고정 대상 검증 · 보상 챕터) · balance(`commission_gold_chapter_mult`). **로드 시 검증한다**(어긋나면 throw): 틀 id 유일 · 종류가 실재 · **`axis` 가 어휘에 있고 그 종류의 `rides_on` 과 맞는다**(처치 = `battle` · 수집 = `yield`) · `ref` 는 `-`(굴린다) 또는 그 어휘의 값(챕터 = 몬스터 표에 있는 챕터 · 종족 = 종족 표 · 개체 = 몬스터 id · 몬스터 등급 · 희귀도 = 아래 어휘) · `need` · `reward_gold` 는 1 이상 정수 · 가중치 ≥ 0 이고 합 > 0 · 등급 id 유일 · 가중치 ≥ 0 이고 합 > 0 · 배수 > 0 · 이름 ko/en · **몬스터 표의 모든 `monster_type` 이 종족 표에 있다** · 챕터 배수 > 0.

**대상 어휘(`AXES`)는 코드가 든다** — 무엇을 세는지가 코드이기 때문이다(construction 의 `TARGETS` 와 같은 자리). 어느 어휘를 얼마나 자주 쓰는지는 틀 표의 행이 정한다.

| `axis` | 종류 | `ref` | 채우는 것 |
|---|---|---|---|
| `chapter` | 처치 | 챕터 번호 | 그 챕터 스테이지에서 잡은 몬스터(등급 무관) |
| `race` | 처치 | `monster_type` | 그 종족 몬스터 |
| `monster` | 처치 | `monster_idx` | 그 몬스터(정예로 나온 것 포함) |
| `grade` | 처치 | `elite` · `boss` | 정예 / 보스(`stage_boss` · `chapter_boss`) |
| `rarity` | 수집 | `magic` · `rare` | 그 희귀도의 드롭 |

| 시스템 export | 계약 |
|---|---|
| `templates` · `grades` · `races` | 정규화한 표 `{id, kind, axis, ref, need, gold, weight}` · `{id, weight, needMult, goldMult, name}` · `{id, name}` — 화면이 등급 이름 · 종족 이름을 읽는다 |
| `roll(rng, ctx)` | `→ {tpl, kind, axis, ref, need, grade, gold}` · 굴릴 틀이 없으면 `null`. `ctx = {chapters: [n], races: [id], monsters: [id], chapter}` — **지금 들어갈 수 있는 것만**(부르는 쪽이 모은다 · 순서가 계약 — 챕터 · 몬스터 오름차순 · 종족은 종족 표 행 순서) · `chapter` = 진행 챕터. **굴릴 수 있는 틀** = `ref` 가 `-` 면 그 어휘의 풀이 비지 않은 것 · 고정이면 그 값이 풀에 든 것(몬스터 등급 · 희귀도는 언제나). `need = max(1, round(틀 need × 등급 needMult))` · `gold = round(틀 gold × 등급 goldMult × commission_gold_chapter_mult^(보상 챕터 − 1))` — **보상 챕터** = 지역이면 그 챕터 · 개체면 그 몬스터의 챕터 · 나머지는 진행 챕터(가야 할 곳이 보상을 정한다). rng 순서는 §5-2 |
| `matchKill(card, ev)` | `→ bool` — `ev = {chapter, monster, race, grade}`(처치 하나). 처치 카드가 아니면 `false` |
| `matchDrop(card, item)` | `→ bool` — 수집 카드이고 `item.rarity === ref` |
...
| 코드 | 뜻 | 내는 곳 |
|---|---|---|
| `missing` | 대상 없음 / 가방에 없음 | equip · unequip · salvage · toggleParty · hire · **upgradeItem** · **dismiss** · **searchSend** · **setStageLevel** · **makeItem**(없는 부위 · 레벨 · 종류) · **swapHeroes** · **makePotion**(표에 없는 물약) · **selectPreset** · **setPotionSlot** · **swapPotionSlot** · **canDepart**(없는 편성) · **construct**(없는 건물 · R137) · **gambleSpin**(표에 없는 판돈 단계 · R149) |
| `class` | 직업 전속 무기군 아님 | equip |
| `bagFull` | **인벤토리** 초과 | equip · unequip · **moveToBag** · **makeItem** |
| **`stashFull`** | **창고 초과** | **moveToStash** (2026-09-11) |
| `full` | 파티 정원 · **도는 부대 정원**(`limitsOf(state).expeditions` — 2026-09-23 다부대) | toggleParty · **canDepart** |
| **`busy`** | **다른 부대에서 싸우는 중** — 나가려는 편성에 **이미 다른 부대로 나간 영웅**이 들어 있다 [신설 2026-09-23 · 다부대] | **canDepart** |
| `locked` · `noParty` | 스테이지 잠김 / **그 편성의** 파티가 비었다 · **마스터리 해금 레벨 미달** · **전술 칸 미해금** · **지금 제작이 안 열린 물약**(R103) · **자물쇠가 걸린 아이템**(2026-09-21 · ADR-0185) | canDepart · learnMastery · **toggleTacticLock** · **makePotion** · **salvage** · **construct**(건설 문턱 미달 · R137) · **gambleSpin**(그 판돈 단계가 안 열렸다 · R149) |
| `gold` · `roster` | 골드 부족 / 로스터 정원 | tavernReroll · hire · **rerollTactic**(`gold`) · **upgradeItem**(`gold`) · **searchTake** · **makePotion**(`gold`) · **construct**(`gold` · R137) · **gambleSpin**(`gold` — 판돈 · R149) |
...
| **`full`** · **`notDone`** (의뢰) | 받은 의뢰가 칸(`commissionSlots`)에 닿았다 / 아직 다 안 채웠다 [2026-09-24 · R153 — 코드는 위 행들과 같은 문자열이다] | **commissionTake**(`full`) · **commissionClaim**(`notDone`) · `missing` = 그 번호의 카드가 없다(**commissionTake** · **commissionClaim** · **commissionDrop**) |
| **`unbuilt`** | **그 기능을 여는 건물(랭크)을 아직 안 지었다** — 기능 자리마다 `hasFeature` 가 막는다. 화면은 `needOf` 로 「무엇을 지어야 하나」를 말한다 [신설 2026-09-22 · R137 · 건설 2단계] · ⚠ 물약 단계 `locked`(makePotion — 단계가 안 열렸다)와 다른 코드다 · **분해 · 알아서 분해는 이 코드를 안 낸다**(건물 없이 열려 있다 · 2026-09-23 · R140) | ~~canDepart~~(2026-09-25 · R161 — 원정은 처음부터 열림) · ~~setStageLevel~~(2026-09-24 · R152 — 처음부터 열림) · **upgradeItem** · **makeItem**(`makeState.err`) · **moveToStash** · **hire** · **tavernReroll** · **searchSend** · **gambleSpin**(R149) · **commissionTake** · **commissionFill**(R153) |
...
```
{
  version: 38, seed: uint32, createdAt: ms, savedAt: ms,
  playMs: ms,                             // **누적 플레이 시간** [2026-09-25 · 버전 무변경 — 아래]. 더하는 것은 `addPlayTime` 하나 · 언제 더할지(보이는 동안만)는 화면이 정한다(SCREEN_DESIGN §2 · ADR-0356)
  ...
  presets: [ { party: [ heroUid ], formation: { tpl, ranks: [[uid...],[uid...]] }, potionSlots: [ potionId | null ], tactics: { slots: { 칸번호: {id, grade} }, locked: [ 칸번호 ] } } ],
                                          // **편성 — v32** [2026-09-21 · R122 · R124]. 길이 = `limitsOf(state).presets`(원정 랭크가 연다 · R156) · 편성 번호 = 자리 + 1.
                                          //   `party` = **편성한 순서 그대로** — `party[0]` 이 리더 · 새 게임은 전부 `[]` (2026-09-09).
                                          //     **한 영웅은 한 편성에만 든다 — v38** [2026-09-23 · 다부대 · GAME_DESIGN §1-1] — ~~한 영웅이 여러 편성에 든다~~ 대체.
                                          //     편성이 곧 부대라 겹치면 두 부대가 같은 사람을 데려간다 — `toggleParty` 가 넣을 때 **다른 편성에서 뺀다**(거절이 아니라 이동) · 로드도 같은 규칙으로 걸러 낸다
  ...
  counters: { hero, item, battle, tavern, tactic, upgrade, search, make, gamble, commission },   // uid 발급·시드 파생의 유일한 출처 · upgrade 는 R95(2026-09-15)부터 안 오른다 — 자리만 남는다 · **make** = 제작 스트림의 회차(R96 · 없으면 0) · **gamble** = 도박장에서 돌린 판 수(R149 · 없으면 0) · **commission** = 굴린 의뢰 카드 수(R153 · 없으면 0)
  commissions: { cards: [ { no, tpl, kind, axis, ref, need, grade, gold, have? } ] },   // **의뢰 게시판** [2026-09-24 · R153 · 버전 무변경 — 아래]. `cards` = 자리 순서 그대로(받은 카드도 제자리) · **굴린 순간의 내용을 박는다**(챕터가 열려도 서 있는 카드는 안 바뀐다) · `have` 가 있으면 받아 둔 것(센 수) · 없으면 게시판에 걸린 것. 명성은 없다(보류 · base_expedition_design §1-3)
  runs: [ { stageId, preset, repeat, lastAt, durationSec, active, fallen? } | null ],   // **부대마다 하나 — v38** [2026-09-23 · 다부대 · GAME_DESIGN §1-1] — ~~`run: {...} | null`(단수)~~ 대체. **길이 = `presets` 와 같고 자리 + 1 = 편성 번호**(부대 = 편성이다 — 별도 부대 번호가 없다) · 안 나간 편성은 `null`. 동시에 `active` 일 수 있는 수는 `limitsOf(state).expeditions` 가 정한다.
                                          //   **`active` — v25** = 그 부대가 도는 중 · **`preset` — v32** = 나간 편성 번호(자리와 같다 · 반복이 그 편성으로 다시 나간다) · **`fallen` — 2026-09-21 · R130** = 이 런에서 쓰러져 있는 영웅 uid(`stepRun` 이 채운다 · `departRun` 이 비운다 · 없으면 `[]` — `active` 일 때만 읽는다: 장비 · 스킬 트리 잠금 `downed`). ~~`downed`~~(v17 삭제 — 옛 「출정 아웃」)와 **다른 필드다**.
                                          //   런 핸들(전투 안의 상태)은 세이브에 없으므로 **불러온 세이브에 `active` 가 선 부대는 전부 끊긴 것이다**(`closeRun` 이 모든 부대를 끊는다)
  reports: [ report ],                    // **리포트 목록 — v21**. 최신이 맨 앞 · 상한 [balance.csv:report_keep] · 새 게임은 `[]`
                                          ...
                                          //   **`report.preset` — v38** [2026-09-23 · 다부대] = 그 런을 낸 편성 번호. 부대가 여럿이면 `reason: null` 인 리포트도 여럿이라 **`at` 만으로는 어느 부대의 것인지 가릴 수 없다** — `liveReport(state, no)` 가 이 필드로 찾는다
  notice: { kind: 'runClosed', runs: [ { stageId, at } ], stageId, at, seenAt } | null,   // **`runs` — v38** [2026-09-23 · 다부대] = `closeRun` 이 끊은 부대마다 한 줄(편성 번호 순) · `stageId` · `at` 은 **그중 첫 줄**을 그대로 둔 것이다(한 부대만 돌던 화면이 안 깨지게)
...
- **`counters.gamble` 도 버전을 안 올리고 들어왔다** [2026-09-24 · R149] — 없으면 `0`(= 돌린 적이 없다)이고 그것이 새 게임의 초기 상태와 같아 소급할 판단이 없다. `deserialize` 끝의 기본값 보정 한 줄이 곧 이관이다. 같은 날 함께 들어왔던 **충전 시계 `gamble` 은 버전을 안 올리고 나갔다** — 횟수 제한이 없어져 읽는 곳이 없다. `deserialize` 가 필드를 **지운다**(`codexCards` 와 같은 문법)
- **`commissions` · `counters.commission` 도 버전을 안 올리고 들어왔다** [2026-09-24 · R153] — 없으면 `{cards: []}` · `0`(= 게시판을 굴린 적이 없다)이고 그것이 새 게임의 초기 상태와 같아 소급할 판단이 없다. 빈 게시판은 선술집을 그릴 때 `commissionFill` 이 채운다. 로드는 모르는 어휘(`axis`)의 카드를 지운다 — 표가 바뀌어 셀 수 없는 카드가 자리를 막지 않게
- **`playMs` 도 버전을 안 올리고 들어왔다** [2026-09-25 · ADR-0356] — 없으면 `0` 부터 센다. 흘러간 플레이 시간은 어디에도 기록이 없어 이관이 소급할 판단이 없다. `deserialize` 끝의 기본값 보정 한 줄(0 이상 유한수가 아니면 `0`)이 곧 이관이다
...
- **타임라인은 세이브에 없다**
- **버전 정책** — 올릴 수 있는 버전은 `deserialize` 안에서 올리고, 못 올리는 버전은 throw. 렌더러는 throw 를 잡아 시작 화면에 사유를 보여준다 — 이 처리는 렌더러의 책임이지 state.js 의 계약이 아니다. **지금 여는 버전은 v38 하나다**(아래)

**v38 에서 또 끊었다** [2026-09-23 · 사용자 지시 「끊어」 · 다부대] — `deserialize` 는 **v38 만 연다.** v1 ~ v37 은 throw 하고 렌더러가 시작 화면에 「이전 형식」 한 줄을 세운다 — **새 게임을 시작해 저장된 순간부터** 이어하기가 된다(SCREEN_DESIGN §3). ~~v37 에서 끊었다~~ [2026-09-22 · DEV_PLAN R139] 와 같은 처리이고, v37 은 하루만 살았다.
...
- ~~**끊는 것은 이번 한 번이다** — v38 이 생기면 `upgradeV37` 을 `deserialize` 안에 두고 한 단계씩 올린다~~ → **2026-09-23 에 한 번 더 끊었다** [사용자 지시] — 다부대가 `run`(단수)을 `runs`(부대마다)로 바꾸는데, 어제 이관 틀을 통째로 지운 직후라 그것만 되살리는 것보다 끊는 쪽이 깨끗하다. **v39 가 생기면** 그때 `upgradeV38` 을 두고 한 단계씩 올린다(위 「버전 정책」)
- **`deserialize` 끝의 기본값 보정은 남는다** — 버전을 안 올리고 들어온 필드(위 불릿들)는 v38 안에서도 없을 수 있다
...
| n번째 전투 | `deriveSeed(seed, counters.battle)` (선증가) | state.departRun — 한 원정의 모든 라운드가 이 스트림 하나를 이어 쓴다(`resolveBattle` 도 이것을 지난다 · R89). **부대가 여럿이어도 카운터는 하나다** [2026-09-23 · 다부대] — 출발할 때마다 선증가라 **부대마다 다른 스트림**을 받고, 각 런은 제 rng 인스턴스를 클로저에 들어 **동시에 돌아도 서로 안 섞인다**(`createRun`). 어느 부대가 몇 번을 받는지는 **출발 순서**가 정하므로, 같은 세이브에서 같은 순서로 보내면 같은 결과다 |
...
| **선술집에서 온 영웅의 시작 장비** | `deriveSeed(seed ^ 0x6EA2, counters.hero)` (`addHero` 가 올린 뒤 — **그 영웅의 번호**) | state.hire · searchTake [신설 2026-09-25 사용자 지시] — 명단(`^ 0x5A17`) · 수색 결과(`^ 0x5EA7`)와 갈라 두어 **나올 영웅을 안 바꾼다**. 번호가 정하므로 불러오기를 다시 해도 같은 한 벌이다 |
...
| **도박장 한 판** | `deriveSeed(seed ^ 0x6A3B, counters.gamble)` (선증가) | state.gambleSpin [신설 2026-09-24 · R149] — 다른 어느 수열과도 안 섞인다. **판 번호 하나로 결과가 정해진다** — 결과를 저장하지 않아도 같은 세이브는 같은 판을 낸다(새로고침으로 되굴릴 수 없다). 거절이면 스트림을 안 연다 |
| **의뢰 카드 한 장** | `deriveSeed(seed ^ 0xC0DA, counters.commission)` (선증가) | state.commissionFill · commissionClaim · commissionDrop [신설 2026-09-24 · R153] — 카드마다 제 스트림이다. 굴린 내용은 **저장한다**(대상 풀이 굴린 순간의 진행에 달려 있어 다시 굴리면 다른 카드가 된다 — 수색 스냅샷과 같은 이유) |
...
| `state.newGame`(시작 장비) · **`state.hire` · `state.searchTake`** | 선술집에서 온 영웅도 **같은 한 벌을 같은 순서로** 굴린다 — 스트림만 다르다(§5-1 · 2026-09-25). 영웅마다 **무기 → 갑옷** [개정 2026-09-14 · R86 — ~~시작 무기 하나 · magic 이라 접미 죄종을 굴리지 않는다~~]. 무기 = 무기군 1회 → `build`(normal — 죄종 0회 · 스킬은 고유 스킬을 뺀 풀에서 1회) · 갑옷 = 베이스 1회(ilvl 1 이라 후보는 클로스 아머 하나 · 2026-09-18) → `build`(normal — 죄종 0회 → 고정 1 → 공통옵션 `armor_common_opt_normal` × 3 · ~~접사 수 → 접사마다 (정의 → 값) → implicit 1회~~ 2026-09-18) |
...
| **`gamble.spin`** | **칸마다 심볼 1회**(칸 번호 순 — 릴 먼저 · `slot_symbol.csv` 행 순서로 가중 추첨) → **코인 칸마다 값 1회**(칸 번호 순 · `slot_coin.csv` 의 가중치 > 0 행) → (코인이 `gamble_hold_trigger` 개 이상이면) **리스핀마다 빈 칸마다 「코인이 서나」 1회 → 서면 값 1회**(칸 번호 순). 라인 판정 · 지급 계산은 0회 [신설 2026-09-24 · R149]. ⚠ **표의 행 순서가 계약이다** — 가중 추첨이 행 순서로 훑는다. 심볼을 늘리거나 줄이면 같은 판 번호가 다른 판을 낸다 |
| **`commission.roll`** | **언제나 3회** — **틀 1회**(굴릴 수 있는 틀만 · `commission.csv` 행 순서로 가중 추첨) → **등급 1회**(`commission_grade.csv` 행 순서로 가중 추첨) → **대상 1회**(`ref` 가 `-` 면 그 풀에서 균등 · **고정이어도 1회 소비하고 버린다**) [신설 2026-09-24 · R153]. ⚠ 표의 행 순서 · 풀의 순서(챕터 · 몬스터 오름차순 · 종족은 `monster_type.csv` 행 순서)가 계약이다. 굴릴 틀이 없으면 0회(`null`) |
...
| 선술집 영웅 장비 시드 솔트 | `0x6EA2` | state.js | 명단 · 수색에서 온 영웅의 시작 장비 — 두 번째 인자는 그 영웅의 번호 `counters.hero` (§5-1 · 신설 2026-09-25) |
| 상점 시드 솔트 | `0x5409` | state.js | 상점 장비 목록 — 두 번째 인자는 방문 회차 (§5-1 · 신설 2026-09-21) |
| 도박장 시드 솔트 | `0x6A3B` | state.js | 도박장 한 판 — 두 번째 인자는 판 번호 `counters.gamble` (§5-1 · 신설 2026-09-24 · R149) |
| 도박장 판돈 · 지급 반올림 | 판돈 = **정수**(`Math.round` — 기본 판돈 × 챕터 배수^(챕터−1) × 단계 배수를 곱한 뒤 한 번) · 골드 지급 = **`floor(goldMult × 판돈)`**(판 하나의 배수 합에 한 번) | state.js gambleState · gambleSpin | 배당표의 골드 칸도 같은 식(`floor(값 × 판돈)`)이다 — 표기 = 지급 [신설 2026-09-24 · R149] |
| 의뢰 시드 솔트 | `0xC0DA` | state.js | 의뢰 카드 한 장 — 두 번째 인자는 카드 번호 `counters.commission` (§5-1 · 신설 2026-09-24 · R153) |
| 의뢰 목표 · 보상 반올림 | 목표 = **정수 · 하한 1**(`max(1, round(need × needMult))`) · 골드 = **정수**(`round(gold × goldMult × 챕터 배수^(챕터−1))` — 곱한 뒤 한 번) | commission.js roll | [신설 2026-09-24 · R153] |
| 의뢰 고정 어휘 `KILL_GRADES` · `RARITIES` | `{elite: ['elite'], boss: ['stage_boss', 'chapter_boss']}` · `['magic', 'rare']` | commission.js | `ref: '-'` 인 몬스터 등급 · 희귀도 틀이 **이 순서로** 풀을 편다(§5-2) — 어느 등급이 「보스」인가를 코드가 알아야 해서 코드에 있다 (신설 2026-09-24 · R153) |
...
- **R151 재촬영 대기** [2026-09-24] — 스킬 표준 모양 개편(PLAN_skill_structure 2단계)은 **런 하나의 `tl` 만** 바꾼다: 시드 7 × 스테이지 104(다른 필드 · `meta.parties` · 나머지 49런 불변). 옛 결투 창 열쇠 충돌(§2-6 「표 셋 분리」 행)을 고친 몫이다 — 몬스터 기사와 영웅 기사가 서로 결투를 걸었다. **지금 찍지 않았다** — 이 변경 전부터 골든이 50/50 빨강이다(`golden.json` 과 입력이 다르다 — CSV 18종 · `balance` 53키 · 이 개편과 무관한 변경). 지금 찍으면 그 변경들이 검토 없이 지문에 박힌다 — 그것들이 정리된 뒤 한 번에 찍는다
...
**도박장 표 넷** [2026-09-24 · R149] — `slot_symbol` · `slot_coin` · `slot_line` · `slot_stake` → `D.slotSymbolRows` · `D.slotCoinRows` · `D.slotLineRows` · `D.slotStakeRows`(원시 행 그대로 — 검증 · 굴림은 `game_logic/gamble.js` · §2-15). 이름은 표의 `_kr`/`_en` 쌍이다.
