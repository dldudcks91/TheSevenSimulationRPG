...
| `kind` | `attack` · `heal` · `buff` · `aura` · `summon` · **`call`** — §2-11 `KINDS`. `call`(불러내기 · 2026-09-18)은 **`owner_kind = monster` 만** 쓰고 `target = self` · `hits` · `mult_pct` · `duration_sec` 0 · `cast_condition = band_missing` 이어야 한다(아니면 throw). **`indirect`**(비직격 · 2026-09-21)도 **`owner_kind = monster` 만** 쓰고 `cool_sec` 0(오오라와 같은 예외) · `hits` 1 · `mult_pct` > 0 · `duration_sec` 0 · `cast_condition` 이 있어야 한다 — **차례로는 안 나간다**(조건이 늘 거짓이라 `pickReady` 가 안 고른다). 실제 발동은 그 사건을 든 쪽이 한다 |
...
| `on_death` | **늘 거짓이다** [신설 2026-09-21] — 차례에 고르지 못하게 막는 자물쇠다. 실제 발동은 `battle.js` 의 `downed` 가 `blast()` 로 직접 부른다(§2-6 `blast`). 「사건이 부르는 스킬」을 선택기를 안 고치고 여는 자리다 |
...
| export | 모양 | 계약 |
|---|---|---|
| `KINDS` | `['attack','heal','buff','aura','summon','call','indirect']` | `skill.csv:kind` 허용값. **`aura`** 는 쿨 없이 상시(§2-6 — 액티브 칸에서 빠져 전투 시작에 `until: Infinity` 창으로 걸린다) · **`summon`** 은 HP 를 가진 유닛을 세운다 [둘 다 2026-09-09] · **`call`** 은 시전자의 무리(§2-13 `band`) 중 서 있지 않은 것을 **한 번에 전부** 세운다(몬스터 전용 · 2026-09-18) |
