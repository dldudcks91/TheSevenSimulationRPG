...
| 코드 | 뜻 | 내는 곳 |
|---|---|---|
| `missing` | 대상 없음 / 가방에 없음 | equip · unequip · salvage · toggleParty · hire · **upgradeItem** · **dismiss** · **searchSend** · **setStageLevel** · **makeItem**(없는 부위 · 레벨대) · **swapHeroes** |
...
| `battle.createRun` 루프 (`simulate` 는 이것을 끝까지) | 틱마다 **창 만료 처리(소비 없음)** → `[...party, ...enemies]` 배열 순서로 `act` |
...
- **재생기는 12종을 전부 안다** (2026-08-30 — `skill`·`heal`·`buff`·`buffEnd`·`regen` 소비 추가 · 2026-09-14 R89 — `card` 삭제 · `refit` 추가). 스킬 칸의 쿨은 `skill` 이벤트의 `ready` 로 걷힌다(첫 준비는 `party[].ready` · `round` 적의 `ready` · `refit` 의 `ready` — R89) — 재생기가 쿨을 **계산하지 않는다**
