...
state.js(deps: hero, item, battle, skill, construction, gamble, balance, …) ──┘
```
...
| **`gambleState(state, now, step = 1)`** | `→ {open, charges, cap, nextAt, chapter, mats: {ore, timber, dust}, stakes: [{step, mult, gold, open}], step, stake, pays: {symbols: [{id, gold, mat}], coins: [{id, gold}]}, n}` | **도박장 화면 상태 한 덩어리** [신설 2026-09-24 · R149 · base_expedition_design 「도박장」] — **판정을 여기서 다 낸다**(`tavernState` 와 같은 규칙) · rng 0 · 상태 불변. `open` = 도박장이 열렸나(선술집 랭크 — `hasFeature('gamble')`). **충전**: `charges` = 지금 쌓인 스핀 수(`0 ~ cap`) · `cap` = `gamble_spin_cap` · `nextAt` = 다음 한 칸이 차는 시각(가득이면 `null`) — 칸은 `gamble_spin_regen_hours` 마다 하나씩 차고 **꺼져 있어도 찬다**(벽시계). `chapter` = 진행 중인 챕터(`shopState` 와 같은 값) · `mats` = 그 챕터 단계의 **재료 id**(광석 = `mine_node.csv` · 목재 = `log_node.csv` 의 그 tier 산출물 · 가루 = `'dust'`) — 그 tier 가 없으면 그보다 낮은 가장 높은 tier. `stakes` = 판돈 단계마다 `gold`(= `round(gamble_stake_gold × gamble_stake_chapter_mult^(chapter−1) × mult)`) · `open`(앞에서부터 `limitsOf(state).gambleStakes` 개). `step` · `stake` = 고른 단계와 그 판돈(없는 단계면 1 단계). `pays` = **그 판돈에서의 배당표** — 골드 심볼 · 코인은 `floor(값 × stake)` 골드 · 재료 심볼은 `pay × mult` 개. `n` = 지금까지 돌린 판 수(`counters.gamble`) |
| **`gambleSpin(state, step, now)`** | `→ {ok, spin}` / `{ok:false, err}` | **한 판** [신설 2026-09-24 · R149]. err: `unbuilt`(도박장이 안 열렸다) → `missing`(표에 없는 단계) → `locked`(그 단계가 안 열렸다) → **`charge`**(충전이 0) → `gold`(판돈이 모자란다) — **이 순서로 본다** · 거절이면 아무것도 안 바뀐다(스트림도 안 연다). 통과하면: 충전 한 칸을 쓰고(§4 `gamble.emptyAt`) · 판돈을 빼고 · `counters.gamble` 선증가 → rng = `deriveSeed(seed ^ 0x6A3B, counters.gamble)`(§5-1) → `gamble.spin(rng)` → **골드 = `floor(goldMult × stake)`** 를 주고 · 재료 = `mats × 단계 배수` 를 준다(광석 · 목재는 `materials[그 챕터 단계 id]` · 가루는 `resources.dust`). `spin` = `gamble.spin` 의 결과 + `{n, step, stake, chapter, gold, mats: {id: n}, net, coinGold: {coinId: 골드}}` — `net` = `gold − stake`(**골드만** — 재료는 순손익에 안 든다) · `coinGold` = 코인마다 **이 판의 판돈에서** 받는 골드(`floor(value × stake)` — 화면이 코인 칸에 적는 값). **같은 세이브 · 같은 판 번호면 같은 결과다** — 결과를 저장하지 않고 다시 부르면 다음 판이다(새로고침으로 되굴릴 수 없다). **장비 · 낙인은 안 나온다**(base_expedition_design 「도박장」) |
| **`gambleSpinAll(state, step, now)`** | `→ {ok, spins, stop, gold, stake, net, mats: {id: n}}` / `{ok:false, err}` | **모두 돌리기** [신설 2026-09-24 · R149] — 거절이 날 때까지 `gambleSpin` 을 거듭 부른다(같은 `now` 라 충전이 도중에 안 찬다 — 끝이 있다). 한 판도 못 돌았으면 그 거절 · `stop` = 멈춘 사유 · 합계(골드 · 판돈 · 순손익 · 재료)는 여기서 낸다(화면이 더하지 않는다). rng 는 판마다 `gambleSpin` 과 같다 |
...
순수 모듈 · 상태 없음 · **세이브를 모른다** — 한 판의 결과를 굴려 **데이터로** 낸다. 충전 · 판돈 차감 · 지급은 `state.js` 가 한다(`gambleSpin` · §2-7). 기획은 [base_expedition_design.md](../game_design/base_expedition_design.md) 「도박장」.
...
| **`charge`** | **도박장 충전이 없다** — 쌓인 스핀이 0 이다(다음 칸은 `gambleState.nextAt`) [신설 2026-09-24 · R149] | **gambleSpin** |
...
| **`unbuilt`** | **그 기능을 여는 건물(랭크)을 아직 안 지었다** — 기능 자리마다 `hasFeature` 가 막는다. 화면은 `needOf` 로 「무엇을 지어야 하나」를 말한다 [신설 2026-09-22 · R137 · 건설 2단계] · ⚠ 물약 단계 `locked`(makePotion — 단계가 안 열렸다)와 다른 코드다 · **분해 · 알아서 분해는 이 코드를 안 낸다**(건물 없이 열려 있다 · 2026-09-23 · R140) | **canDepart** · **setStageLevel** · **upgradeItem** · **makeItem**(`makeState.err`) · **moveToStash** · **hire** · **tavernReroll** · **searchSend** · **gambleSpin**(R149) |
...
```
{
  ...
  counters: { hero, item, battle, tavern, tactic, upgrade, search, make, gamble },   // uid 발급·시드 파생의 유일한 출처 · upgrade 는 R95(2026-09-15)부터 안 오른다 — 자리만 남는다 · **make** = 제작 스트림의 회차(R96 · 없으면 0) · **gamble** = 도박장에서 돌린 판 수(R149 · 없으면 0)
  gamble: { emptyAt: ms | null },         // **도박장 충전 시계** [2026-09-24 · R149 · 버전 무변경 — 아래]. 충전 = min(cap, floor((now − emptyAt) / 주기)) — `emptyAt` = 「충전이 0 이었던」 가상의 시각 · **`null` = 가득**. 한 칸을 쓰면 가득이었을 땐 `now − (cap − 1) × 주기`, 아니면 `emptyAt + 주기` 로 민다(차던 중인 몫을 안 버린다)
...
- **`gamble` · `counters.gamble` 도 버전을 안 올리고 들어왔다** [2026-09-24 · R149] — 없으면 `{emptyAt: null}`(= 충전이 가득) · `0`(= 돌린 적이 없다)이고 그것이 새 게임의 초기 상태와 같아 소급할 판단이 없다. `deserialize` 끝의 기본값 보정 두 줄이 곧 이관이다
