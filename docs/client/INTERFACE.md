...
| `canDepart(state, stageId, now, no?)` | `→ null` / `unbuilt` / `locked` / `missing` / `noParty` / `searching` / **`busy`** / **`full`** | 편성 `no`(기본 **고른 편성**)로 나갈 수 있나 — 이 순서로 본다 [편성 2026-09-21 · R122 · **다부대 2026-09-23**]: **원정이 안 열렸다**(`expedition` — 처음부터 지어짐 · R137) → 스테이지 잠김 → 없는 편성 → **그 편성의 파티가 비었다** → **그 편성에 수색 나간 영웅이 있다**(편성은 계획이라 든 채로 수색을 보낼 수 있고 여기서 막는다) → **`busy` = 그 편성의 영웅이 다른 부대에서 싸우는 중이다**(도는 부대의 인원은 출발 때 굳은 스냅샷이라, 그 영웅을 다른 편성으로 옮겨 두면 한 사람이 두 부대에 서는 셈이 된다 — 여기서 막는다) → **`full` = 도는 부대가 이미 `limitsOf(state).expeditions` 개다**(**그 편성이 이미 도는 중이면 안 센다** — 다시 보내는 것은 그 부대를 끊고 여는 것이라 수가 안 는다). **그 편성의 원정이 도는 중이어도 막지 않는다** — 보내면 `departRun` 이 **그 부대만** 끊는다 [R92] · ~~`injured`~~ 없음 (2026-09-03) |
...
| **`commissionState(state)`** | `→ {open, slots, taken, cards: [{…card, taken, have, done, canTake}]}` | **의뢰 게시판 한 벌** [신설 2026-09-24 · R153 · base_expedition_design §1-3] — **판정을 여기서 다 낸다** · rng 0 · 상태 불변. `open` = 게시판이 열렸나(`hasFeature('commission_board')` — 선술집 r1) · `slots` = `limitsOf(state).commissionSlots` · `taken` = 받아 둔 수. `cards` = **게시판 자리 순서 그대로**(받은 카드도 제자리에 선다 — 자리 수 = `commission_board_cards`) · 카드 = 굴린 순간의 내용 `{no, tpl, kind, axis, ref, need, grade, gold}`(§2-16 `roll`) + `taken`(받아 뒀나) · `have`(센 수 — 안 받았으면 `null`) · `done`(`have ≥ need`) · `canTake`(열렸고 · 안 받았고 · 받은 수 < `slots`) |
...
| **`unbuilt`** | **그 기능을 여는 건물(랭크)을 아직 안 지었다** — 기능 자리마다 `hasFeature` 가 막는다. 화면은 `needOf` 로 「무엇을 지어야 하나」를 말한다 [신설 2026-09-22 · R137 · 건설 2단계] · ⚠ 물약 단계 `locked`(makePotion — 단계가 안 열렸다)와 다른 코드다 · **분해 · 알아서 분해는 이 코드를 안 낸다**(건물 없이 열려 있다 · 2026-09-23 · R140) | **canDepart** · ~~setStageLevel~~(2026-09-24 · R152 — 처음부터 열림) · **upgradeItem** · **makeItem**(`makeState.err`) · **moveToStash** · **hire** · **tavernReroll** · **searchSend** · **gambleSpin**(R149) · **commissionTake** · **commissionFill**(R153) |
