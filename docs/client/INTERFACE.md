...
- `reason` 은 `clear` / `wipe` / `timeout`. **`retreat` 는 2026-09-03 에 폐기됐다.**
  **귀환 룰** (base_expedition_design §1-1 · **개정 2026-09-03**) — 전투불능자가 나와도 **런을 접지 않는다.** 남은 인원으로 계속 가고 **전원이 쓰러졌을 때만** `wipe` 로 끝난다. 판정 순서는 ① 전멸 ② 라운드 정리(클리어·다음 라운드) ③ 제한시간. ~~전투불능자가 하나라도 나오면 그 자리에서 철수~~ 는 폐기 — 그래서 `wipe` 가 **패배의 일반형**이 됐다(종전에는 같은 틱 전멸에서만 났다). 쓰러진 영웅은 `result.downed` 에 실려 `state.run.downed`(출정 누적)로 옮겨간다
...
`export const SAVE_VERSION = 15`  [정정 2026-09-08 — 문서가 v11 에서 멈춰 있었다. 같은 문서 §4 는 이미 v15 이관을 적고 있어 자기모순이었다]
...
| `heroById(state, uid)` · `heroItems(state, h)` · `isOut(state, uid)` | 조회 | `isOut` = **이번 출정에서 아웃**됐는가. 시계를 안 본다 — 「나가 있는 동안 참 / 전투 밖으로 나오면 거짓」 두 상태뿐이다 (2026-09-03 · 09-06 어휘 정리) |
...
| `returnToTown(state)` | 부작용 | `→ {healed: uid[]}`. **마을 귀환 — 아웃된 영웅이 전부 낫는다.** 시계를 안 받는다(대기가 없다). 출정이 끝나는 세 자리에서 부른다: 반복이 안 이어질 때 · 전멸 · `closeRun` |
| `activeParty(state, stageId)` | 조회 | 이번 런에 **실제로 나가는** uid 목록. 같은 스테이지를 반복으로 잇는 중이면 아웃을 뺀다 — 새 출정이면 전원이다(떠나는 순간이 곧 회복) |
...
```
{
  version: 15, seed: uint32, createdAt: ms, savedAt: ms,
...
- **HP 는 세이브에 없다** — 매 전투 최대치 시작. 전투불능은 `run.downed`(**이번 출정 누적 아웃**) 하나이고 **전투 밖으로 나오면 비워진다**. 회복 대기라는 상태는 없다 (base_expedition_design §1-1)
- **타임라인은 세이브에 없다**
...
| `battle.act` 기본 공격 [확정 2026-09-08] | 타겟 1회(**도발 중이면 0회**) → `strike`. 대상은 **생존 적 중 균등 무작위** — 영웅·몬스터 양쪽이 같은 규칙이다 (battle_design.md §3-1) |
| `battle.act` 스킬 | 발동 선택 0회 → `enemy_single`: 타겟 1회(도발 무관 — 파티 스킬은 도발 대상이 아니다) → `hits` 회 `strike` / `enemy_rotate`·`enemy_chain`: 시작점 1회 → 타격마다 `strike` / `enemy_all`: 0회 → 대상마다 `strike` / `heal`·`buff`: 0회. ⚠ **여러 대상을 때리는 스킬은 대상마다 `strike` 를 부른다** — 적중·치명을 대상마다 따로 굴리는 것이 계약이고, 그래서 **타격 rng 소비가 대상 수에 종속**된다 [확정 2026-09-08 · battle_design.md §3-1] |
