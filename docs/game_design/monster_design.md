...
- [ ] 파티 전투 전제의 **수치 재스케일** — 구조는 확정됨 (스테이지 9라운드·라운드당 1~3마리 = base_expedition_design.md §1-2, 발동 규칙 = battle_design.md). 남은 작업: `battle_time_target`을 라운드 전체 목표 시간으로 재해석하고 HP/ATK를 3인 파티 DPS 기준으로 재스케일

---

*마지막 업데이트: 2026-08-21 (파티 전투 재설계 항목 갱신 — 구조 확정, 수치 재스케일만 잔존)*
