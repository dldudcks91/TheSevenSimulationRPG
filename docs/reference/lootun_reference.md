...
> 본작도 같은 문제를 다르게 푼다 — **양손 무기의 죄종 접사 = 2포인트**(item_design.md §2). *즉 두 게임 모두 "양손은 보상해야 한다"에 도달했고, Lootun은 수치로, 본작은 세트포인트로 갚는다.*
...
> **본작에 없는 개념.** 파밍 게임의 고질적 문제(레벨 오르면 좋은 아이템도 버려짐)를 슬롯 하나로 우회했다. 본작 `유니크 19종 데이터화`(item_design.md §5) 논의 때 검토할 만한 형태.
...
> **본작에는 이 고리가 통째로 없다.** 본작의 강화 비용은 골드 단일이고, 드롭은 "쓰거나 버리거나"뿐이다 — item_design.md §2 드롭 파이프라인의 **가장 큰 빈 구멍**.
