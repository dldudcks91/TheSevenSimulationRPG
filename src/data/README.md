...
| `weapon_sin_option.csv` | 12 | **무기 죄종 칸 후보** (item_design §1 「무기 옵션」 · 2026-09-11 R78) — `sin` · `applies_to`(`all` / 무기군 `damage_kind` / 직업 id — 시기 칸이 물리 · 마법사 · 사제로 갈린다) · `stat` · `scale`(`flat` / **`fine`** = 소수 1자리) · `min` · `max`. 한 죄종 · 한 무기군에 행이 여럿이면 그중 하나를 굴린다(탐욕 셋 · 시기-사제 둘). 로드 검증은 `item.js`. ⚠ 수치는 전부 제안 · 행 순서가 결정론에 걸린다 |
...
| `potion.csv` | 5 | **물약 단계** (battle_design §7-1 · item_design §7-4 확정 2026-09-15 · R103 · ⚠ **값은 사용자 지정 임시값**). `potion_id` · `kind`(지금은 `heal` 하나 — 모르는 종류는 로드에서 멈춘다) · `tier`(종류 안에서 1 부터 연속) · 이름 `_kr`/`_en`(**영어가 원본 · Diablo 2 이름 · 한글은 음차**) · `heal`(정해진 회복량 — 단계마다 커진다) · `craft_gold`(제작 골드 — 지금은 재료 없이 골드만) · `craftable`(0/1 — ⚠ **지금 제작이 열린 단계** · 해금 조건은 기획 미정이라 임시 칸) · `start_owned`(0/1 — 새 게임이 갖고 시작하는 물약). 칸 수 · 마시는 HP 비율 · 쿨은 `balance.csv:potion_*` 가 든다. 검증은 로드 시 `state.js` |

**⚠ 배열 순서가 곧 계약인 표들** — `equip_slot`(부위) · `item_base`(부위별) · `affix` · `hero_name` · `hero_trait` · **`skill`**(고유 스킬 풀 — 09-01) · **`hero_tier`**(등급 굴림 — 09-08) · **`weapon_sin_option` · `weapon_common_option`**(무기 옵션 — 09-11) 은 코드가 `rng` 로 **인덱스를 굴린다**. 행을 재정렬하면 같은 시드가 다른 결과를 낸다 ([INTERFACE §5-2](../../docs/client/INTERFACE.md)). 행 추가는 **끝에** 한다.
