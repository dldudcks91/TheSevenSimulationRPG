...
| `skill.csv` | 22 | **전직 임시분 14**(08-27 판 · 전사 ③ 미정 · `owner_kind=advance` — 2026-09-03 `job` 에서 명칭 교정) + **무기군 8**(2026-09-03 ⚠임시 신설 · `owner_kind=weapon_group` · `owner_id` = `weapon_group.csv` 행 · `innate_pool=0` 이라 고유 굴림에 안 들어간다) — **`owner_kind,owner_id`** 가 출처(`job`/`advance`/`weapon_group`/`unique`) · 종류·타겟·타수·배율·감쇠·쿨·지속·원소·버프 스탯·발동 조건·`status`(코드 미독)·**`tags`(영문 id · `\|` 구분 최대 2 — `skill.js` 가 검증하고 전투는 안 읽는다. skill_design §11)**·우선순위 · **레어 고유 스킬 풀**(영웅 생성 시 전 행에서 1개 — hero_design §1 프로토타입 09-01. ⚠ 행 순서가 결정론에 걸린다) · **`icon`·`desc_kr`·`desc_en`**(플레이어 표시 — 종전 `ui/mock.js:SKILL_DISPLAY`) · **`innate_pool`**(0/1 — 고유 풀 소속. 「회복은 풀에서 뺀다」 같은 결정을 코드 없이 내리는 손잡이) · **`note`**(설계 노트 — 옛 `description_kr`. 플레이어에게 안 보인다) | **액티브 3칸은 출처가 정한다**(고유 · 무기군 · 전직 — skill_design §2 · DEV_PLAN R35).
...
| `commission_kind.csv` | 4 | **의뢰의 종류** (GAME_DESIGN §9 확정 09-03) — 사냥 · 파견 의뢰 · 약탈 · 보호. `form` = **`count`(세는 형 — 받아두면 평소 활동이 채운다) / `go`(가는 형 — 파티를 보낸다)** · `channel` = 어디서 도는가(`expedition`\|`dispatch`\|`realtime`\|`offline`). **이름과 「어떻게 도는가」 줄이 `_kr`/`_en` 쌍으로 여기 있다** — 화면은 i18n 이 아니라 이 표를 `L()` 로 푼다 (SCREEN_DESIGN §14) |
| `commission.csv` | 7 | **의뢰 게시판의 행** — ⚠ **표 전체가 임시 자리채움이다**(`mine_node.csv` 와 같은 처지). 목표·보상·명성이 기획 미정이라(GAME_DESIGN §10 「의뢰 세부」) 값이 전부 지어낸 것이고, **굴림이 생기면 이 표가 템플릿 표가 된다.** **칸 수 = 행 수** — 화면이 개수를 안 박는다(`tactic_slot` 과 같은 문법) |
