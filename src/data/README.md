...
| 파일 | 행 | 내용 |
|---|---|---|
| `balance.csv` | 153 | `key,value,status,knob,description_kr`. `knob=1` **86** (2026-09-15 재집계) = [`src/dev/README.md`](../dev/README.md) 의 밸런스 손잡이 표. ⚠ **이 두 수는 잘 낡는다** — 키가 늘고 주는 것이 일상이라, 세야 할 일이 있으면 파일을 세는 쪽이 빠르다(2026-09-09 재집계: 옛 「109행 · 손잡이 48」은 오래 뒤처져 있었다) |
