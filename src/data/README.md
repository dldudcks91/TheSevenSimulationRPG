...
| 파일 | 행 | 내용 |
|---|---|---|
| `balance.csv` | 130 | `key,value,status,knob,description_kr`. `knob=1` **77** = [`src/dev/README.md`](../dev/README.md) 의 밸런스 손잡이 표. ⚠ **이 두 수는 잘 낡는다** — 키가 늘고 주는 것이 일상이라, 세야 할 일이 있으면 파일을 세는 쪽이 빠르다(2026-09-09 재집계: 옛 「109행 · 손잡이 48」은 오래 뒤처져 있었다) |
| `chapter.csv` | 7 | 챕터 id · 죄종 · 이름 `_kr`/`_en`. 몬스터 id 앞자리 = 챕터 (1101 → 1) |
| `stage.csv` | 35 | 스테이지 — 이름 `_kr`/`_en` · `bg`(계승 배경 자산 유무) · 타입 / dlvl / 보스 / 보스 등급 · **`round_set`**(`stage_round.csv` 의 세트 — 챕터의 마지막 스테이지는 `boss_only` · 2026-09-11) · **`story_kr`/`story_en`**(출정 창 「이야기」 칸의 입장 텍스트 · 2026-09-14 — 원고 각 장 「스테이지별 입장 텍스트」를 **스테이지 번호로** 옮겼다: 원고 「보스 스테이지」 글 = 스테이지 4 · 스테이지 5 는 원고 대면 절에서 뽑은 초안 · `_en` 은 직역 · 원고의 쉼표는 CSV 계약상 걷었다. `game_logic` 은 안 읽는다) |
...
| `log_node.csv` | 7 | **벌목지 — 단계 1~7** (⚠ **표 전체가 구조 검증용 임시값이다**). 위와 같은 모양 · 컬럼은 `log_id` · `timber_id`/`timber_name_kr`/`timber_name_en`. **담당 능력치는 건강 · 산출 계열은 목재**로 기획 확정이고 **소모처는 크래프트 경제(백지)** 가 정한다 — ⚠ 그때 지켜야 할 조건 하나: **목재가 광석과 같은 소모처만 먹으면 채광과 벌목이 같은 통화가 되어 통화 비중복이 흐려진다**(base_expedition_design §2-2). ⚠ **`game_logic` 이 아직 안 읽는다** |
