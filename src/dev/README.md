...
| `?bg=<스타일>` | **스테이지 배경 아트 스타일 교체** [2026-09-16] — `src/assets/art/backgrounds/<스타일>/`. 목록은 `ui/mock.js:BG_STYLES`(**`illustrate` 정식** · `pixel` 비교용 · ADR-0152) · `?face=` 와 같이 localStorage 에 남는다. ⚠ 그 스타일에 그림이 없는 스테이지는 그라디언트로 떨어진다(`illustrate` 는 챕터 1 · 2 뿐이라 **챕터 3 은 `?bg=pixel` 로만 보인다**) · **거점 배경 · 탐험 지도는 스타일을 안 탄다** |
