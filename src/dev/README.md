...
| ② | `meta.balance` — `balance.csv` **전 107키** | `키: 옛값 → 새값` 을 최대 8개. 손잡이 5키는 **통과 메시지의 문구일 뿐**이다 |
...
> **`balance.csv:knob = 1` 이 손잡이의 SSOT 다** (48키, 2026-08-31 재집계 — 강화 4키가 들어왔다). 손잡이인지 아닌지를 사람 기억이 아니라 컬럼이 들고 있어야 캘리브레이션이 재현된다.
...
### 마지막 측정 (2026-08-31 3차 — **`hero_hp_base` 230 → 100** · 사용자 지시 · 시드 20 · Lv1 시작 파티 · 방어구 없음 · 전직 액티브 전부 켬)
...
### 직전 측정 (2026-08-31 — **손잡이 재보정: `monster_atk_scale` 0.1 → 0.2** · `hero_hp_base` 230 · 시드 20 · Lv1 시작 파티 · 방어구 없음 · 전직 액티브 전부 켬)
