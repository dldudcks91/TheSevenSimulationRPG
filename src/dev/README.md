...
1. `start.bat` (로컬 http 서버 = `serve.py` — ES Modules 는 `file://` 에서 CORS 로 막힌다).
   `serve.py` 는 `http.server` + **`Cache-Control: no-cache` + ETag** 다 — 아트를 같은 파일명으로 갈아끼웠을 때 브라우저가 옛 그림을 캐시로 계속 쓰던 문제 (2026-09-05). 쓸 때마다 재검증하고 안 바뀐 파일은 304 라 다시 그려도 그림을 다시 받지 않는다 (2026-09-24 · 옛 `no-store` 는 본문째 다시 받았다). 헤드리스 검증이 `--disk-cache-size=1` 을 주는 것과 같은 목적(옛 파일을 안 쓴다)
...
| `?dev=play` | 관전 — `&bt=log|dmg` 면 **그 판을 고른 채**(보이는 것은 나눔 배치뿐 · ADR-0130), `&lay=split` 이면 **나눔 배치** (2026-09-03) · **`&logf=party|enemy` 면 로그를 그 주체로 거른 채**(2026-09-15 · ADR-0131) · **`&dmgf=taken` 이면 누적 판을 받은 피해 탭으로 고른 채**(2026-09-21 · ADR-0252) · **`&tab=<탭>` 이면 관전을 켠 채 그 탭**(2026-09-08 — 런이 도는 동안의 다른 탭 화면. 예: 영웅 띠의 「원정 중」 라벨) · **`&rep=1` 이면 반복 원정을 켠 채**(2026-09-10 — 반복은 전진 패널 토글로만 켜져서 「런이 끝나면 다음 런이 선다」에 못 닿았다 · ADR-0074) · **`&tip=e` 면 첫 적 카드 · `&tip=p` 면 첫 영웅 카드의 툴팁이 뜬 채**(2026-09-14 — 유닛 툴팁은 hover 로만 뜬다 · SCREEN_DESIGN §2 「유닛 툴팁 규격」) · **`&alt=1` 을 더하면 Alt 를 누른 채**(오른쪽 세부 옵션 열) — **`&eq=n` 을 더하면 n번째 찬 장비 칸에 올린 채**(「착용 중」 아이템 카드 · 2026-09-23 · ADR-0305 — 칸 hover 도 Alt 동안만이다 · 런이 라운드를 넘기면 툴팁이 닫히므로 `--virtual-time-budget` 을 짧게) · **`&stage=<id>` 면 그 스테이지**(앞 스테이지를 클리어 처리하고 보낸다 · 2026-09-15 — 오오라를 든 몬스터는 2장부터라 첫 스테이지로는 못 본다 · ADR-0127 · **2장 이후면 원정 건물을 끝까지 짓는다** — 장은 원정 랭크가 연다 · 2026-09-24 · R152) · **`&parties=n` 이면 부대 n 개가 동시에 도는 관전**(2026-09-24 — 남는 영웅을 편성 2 · 3 에 한 명씩 넣어 보내고 보는 부대는 1부대로 되돌린다 · 관전 칸이 부대마다 서는 것과 「안 보는 부대도 앱 시계가 민다」는 클릭으로는 편성마다 넣고 따로 보내야 닿는다 · 상한은 `limitsOf.expeditions` · ADR-0316) |
| `?dev=search` | 선술집 — 수색 (`&s=out` 갓 보냄 · `&s=mid` 절반·**만남** · 기본 완료 · `&key=1` 소문에 맞는 죄종을 보냄 → 열쇠 답 · `&ans=n` n번째 답을 미리 고름) |
| `?dev=gamble` | 선술집 — **도박장 창이 열린** 화면 (2026-09-24 · SCREEN_DESIGN §8-1 · ADR-0322). 창은 클릭으로만 열리고 도박장은 선술집 랭크가 열어 **선술집을 끝까지 지은 채**다. `&open=0` 이면 창을 닫은 채 **열린 칸**(새 게임의 `?tab=tavern` 은 잠긴 칸) · `&spin=1` 한 판을 돌려 **멈춘 판** · `&hold=1` **홀드 앤 스핀이 선 판**(판 번호를 홀드가 서는 판까지 민다 — ADR-0331 · R149) · `&all=1` **n판 돌리기의 합계**(ADR-0334) · `&roll=ms`(`&spin=1` 에 더한다) **도는 판을 그 순간에 세운 채** — 릴 띠가 흐르는 장면 · 왼쪽부터 멈추는 장면(ADR-0333). 판을 돌리는 길은 판돈을 n판치(`gamble_batch_spins`) 채워 준다 |
| `?dev=commission` | 선술집 — **받아 둔 의뢰가 선** 게시판 (2026-09-24 · SCREEN_DESIGN §14 · ADR-0352). 선술집을 끝까지 짓고 받을 수(`limitsOf.commissionSlots`)만큼 앞 카드를 받는다 — 첫 장은 반쯤(진행 막대) · 둘째 장은 다 채운 채([수령]) · **받을 수가 하나면 `&done=1` 로 첫 장을 다 채운다**([수령] · 2026-09-25). `&arm=1` 이면 첫 장의 [포기]를 한 번 누른 채(「정말 포기」) |
...
| `&cn=max` | **모든 건물을 끝까지 지은** 판 (R137 · SCREEN_DESIGN §10) — 새 게임은 제련소 · 선술집이 안 지어져 그 탭 · 기능에 곧장 못 닿는다. 문턱 · 비용을 건너뛰고 편성 수 · 물약 칸도 따라 는다. `?dev=search` · `?dev=gamble` · `?dev=form&lvl=n` · `?dev=mats` 는 제 건물만 끝까지 짓는다 |
| `?dev=tip` | **아이템 툴팁이 떠 있는** 캐릭터 탭 (2026-09-10) — 툴팁은 hover 로만 뜬다. 한 런을 정산해 **주운 것을 다 입히고** 한 번 더 정산해 **가방에 비교 상대가 있는 물건**을 남긴다(새 게임은 몸에 무기 하나뿐이라 비교 두 장에 못 닿는다). `&i=n` 이면 n번째 찬 칸(기본 1 — 가방에 무엇이 떨어질지는 시드가 정하므로 무기 칸을 골라 잡는 유일한 길) · `&t=doll` 이면 페이퍼돌 칸(한 장) · `&t=skill` 이면 액티브 카드의 **스킬 설명창**(`&i=n` 이 n번째 칸 · ADR-0118) · **`&t=potion` 이면 편성 탭 물약 칸의 물약 카드**(이 길만 탭이 편성으로 갈린다 — 칸이 거기 있다 · `&i=1` 이 첫 칸(찬 칸) · `&i=2` 부터가 빈 칸 · SCREEN_DESIGN §2 「물약 툴팁 규격」 · ADR-0315) |
