...
| `skill_effects.js` | **「종류」 등록표** — 표준 모양(스킬 → 하는 일 줄 → 걸린 효과 · 2026-09-24 R151): 나가는 방식 3(`CASTS` — 차례에 고르나 · 쿨) · 사건 1(`EVENT_TRIGGERS` — `on_death`) · 하는 일 6(`EFFECT_TYPES` — 실행 함수도 든다 · 불러내기 `call` · 고정 피해 `fixed` 는 몬스터 전용 · 2026-09-22 R136 — ~~종류 `KINDS`~~) · **공격 대상 5**(`ATTACK_TARGETS` · 감쇠의 뜻 `HIT_DECAY`) · **고르는 대상 6**(`PICK_TARGETS` — 아군 · 적에게 거는 창) · 걸린 효과의 능력치 13(`EFFECTS` — 표식은 `state`) · **능력치 표**(창을 능력치마다 모아 파생 — `refreshDerived`) · 발동 조건 3 (INTERFACE §2-11). 종류 하나 = 등록 한 번(어휘와 실행이 같은 표를 읽는다). 상태·rng 없음 |
| `skill_runtime.js` | **액티브 실행** — 시전 한 번(`cast` — 하는 일 줄을 `seq` 순으로 · 표가 실행한다) · 차례(`act`) · 사건(`fire` — 자폭) · 쿨 · 창(열쇠 = 걸린 효과 id) · 배리어 · 회복 · 사건 훅(`reactions`). **스킬 id 전용 분기가 없다**(2026-09-24 R151). `battle.createRun` 이 런마다 만든다(전역 상태 없음) |
...
| `gamble.js` | **도박장 슬롯** — 심볼 · 코인 · 라인 · 판돈 표를 검증하고 **한 판을 굴려 결과를 데이터로 낸다**(판 · 맞은 라인 · 홀드 앤 스핀 걸음 · 판돈의 몇 배 · 재료 개수). **무상태 · 세이브를 모른다** — 판돈 · 지급은 `state.js`(`gambleSpin`)가 한다 · 횟수 제한은 없다 (2026-09-24 · R149 · ADR-0334) |
| `commission.js` | **의뢰** — 틀 · 종류 · 등급 · 종족 표를 검증하고 **게시판 카드 한 장을 굴리며**(틀 → 등급 → 대상) **처치 · 드롭 하나가 카드를 채우는지** 답한다. 대상 어휘(`AXES` — 지역 · 종족 · 개체 · 몬스터 등급 · 희귀도)만 코드가 든다. **무상태 · 세이브를 모른다** — 받기 · 세기 · 수령 · 포기는 `state.js`(`commission*`)가 한다 (2026-09-24 · R153 · INTERFACE §2-16) |
